...
const SPECS: Record<string, RequiredDocSpec[]> = {
  ...
  // 수입허가증이 없다(Circular 25 제10조 — 2마리 이하 동반 면제). 베트남 건강증명서는
  // 한국 APQA 정부수의관이 발급하는 수출검역증이 그 역할을 하므로 별도 서류를 두지 않는다.
  //
  // 베트남 수입 검역증(2026-07-20 추가) — 도착 검역 후 발급된다.
  //   근거: 베트남 공공서비스 포털 행정절차 1.003113
  //   「Cấp Giấy chứng nhận kiểm dịch động vật, sản phẩm động vật nhập khẩu」의 4단계 —
  //   "수의위생 요건을 충족한 동물에 대해 **Mẫu 15a** 서식으로 검역증명서를 발급"(축산물은 15b).
  //   Thông tư 25 가 이 증명서의 유효기간을 30~60일로 규정한 것도 존재를 뒷받침한다.
  //   ⚠️ 한계: Thông tư 25 원문(제10조)은 사이트 차단(403·연결거부)으로 직접 확인 못 했다.
  //   절차 1.003113 은 일반 수입 검역이라, 제10조의 '동반 2마리 이하 → 공항 현장 등록'
  //   경로도 같은 Mẫu 15a 를 쓰는지는 베트남 법무법인(asokalaw) 서술에 의존한다.
  //   대만처럼 우리 www 가이드만 근거인 상태는 아니다(그건 되돌렸다 — 대만 블록 주석 참고).
  //
  // 이름에 나라를 붙인다 — **검역증류 한정 규칙**(사용자 지정 2026-07-20).
  //   서류 목록에는 한국 수출·수입 동물검역증이 늘 함께 있어서(전 목적지 기본), 나라를 안
  //   붙이면 어느 나라 검역증인지 구분이 안 된다. 일본(일본 수입/수출 동물검역증)·중국
  //   (중국 동물위생증명서)도 같은 형태다.
  //   반면 **허가증류는 계속 안 붙인다** — 태국 수입 허가서(R.7)·필리핀 수입 허가증(SPSIC)·
  //   대만 수입 허가증(Import Permit). 목적지 탭이 이미 그 나라이고 한국 것과 겹치지 않는다.
  '베트남': [
    {
    ...
    },
    {
      ...
      roundTripOnly: true,
      description:
        '베트남 수출 검역 후 발급받아요.\n\n정확한 서류 이름은 Giấy chứng nhận kiểm dịch động vật xuất khẩu 입니다.\n\n발급일로부터 15일간 유효하니 출국 일정에 맞춰 받으세요.\n\n한국 입국 때 필요하니 원본을 잘 보관하세요.\n\n앱에 사본 이미지를 저장해두면 관련 정보를 확인할 때 편리해요.',
