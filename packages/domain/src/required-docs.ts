...
const SPECS: Record<string, RequiredDocSpec[]> = {
  ...
  // 베트남 — 별지25호 + 항체결과(귀국용) + 한국 수출/수입 검역증 + DAH 검역 신청서.
  ...
  // 대만 (APHIA) — 수입허가증 + 항체 결과지 + 별지 25호 + 한국 수출검역증 + 대만 수입검역증
  // (+왕복 한국 수입검역증).
  // 도착 검역 증명서는 예전엔 "사전 준비 서류가 아니라"는 이유로 목록에서 뺐는데, 그 기준이면
  // 일본 수입 동물검역증·태국 R.7 도 빠져야 한다(둘 다 도착 후 받는 것인데 목록에 있다).
  // 기준을 통일했다 — **수입 검역이 있고 서류가 발급되면 넣는다**(사용자 지정 2026-07-20).
  // 대만은 발급 사실이 두 곳에 기록돼 있다: www 가이드("도착 시 검사에서 문제가 없으면 수입
  // 검역 증명서가 발급되며, 수입이 허가됩니다") + 이 파일의 옛 주석. 영문 정식 명칭은 확인된
  // 근거가 없어 앱 공통 표기 '{국가} 수출/수입 동물검역증'을 따른다(catalog.ts 수출증과 같은 규칙).
  ...
  '대만': [
    ...
    },
    {
      ...
      group: 'quarantine',
      description:
        '대만 수입 검역 후 발급받아요.\n\n대만에서 출국할 때 필요할 수 있으므로 잘 보관해두세요.\n\n앱에 사본 이미지를 저장해두면 관련 정보를 확인할 때 편리해요.',
      previewStepId: 'departure',
    },
    {
