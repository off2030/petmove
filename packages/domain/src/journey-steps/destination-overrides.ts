  ...
  south_africa: seaPermitOverrides({
    ...
    flight: {
      ...
      description:
        ...
        // ⚠️ '경로 확인'과 '확정 결제'를 **두 줄로 갈라** 쓴다(2026-07-30 사용자 확정). 구 한 줄
        //   ('수입 허가가 나온 뒤에 일정을 확정하세요')은 예약 자체를 허가 뒤로 미루라고 읽혔는데,
        //   검역시설 예약이 **희망 입국일을 먼저 요구**해서(초안: "항공편과 희망 입국일을 정한 뒤
        //   일찍 문의") 그러면 두 카드가 서로를 기다린다. 알아보는 건 먼저, 돈이 묶이는 건 나중.
    ...
    importQuarantine: {
      ...
      helpText: '남아프리카공화국 도착 후 검역을 시작한 날짜',
      attachmentHint: '검역 서류 사본을 사진·PDF로 보관하세요.',
    ...
    extra: {
      ...
      'heartworm-test': {
        title: '심장사상충 예방',
        description:
          // 약 목록은 HA2123 6.1 원문 그대로 6종(2026-07-30 대조). ProHeart SR12 가 빠져 있었는데,
          //   각주 2) 가 "음성 검사일이 주사 후 11개월 이내면 추가 투약 불요"라 **일정 자체가 달라지는**
          //   선택지다 — 빼 두면 매달 먹이는 줄로만 읽힌다.
          '심장사상충 예방을 시작하세요.\n\n음성 검사를 위해 채혈한 날부터 출국일까지 끊기지 않게 투약해요.\n디에틸카바마진·이버멕틴·밀베마이신 옥심·목시덱틴·셀라멕틴 중 증명서가 허용한 약을 써요.\n1년 지속형 주사(ProHeart SR12)를 쓴 경우, 채혈일이 주사 후 11개월 이내면 추가 투약이 필요 없어요.\n약 이름·용량·투여일을 기록해두세요. 건강증명서에 적어야 해요.\n\n남아프리카공화국에 도착한 뒤에도 6개월 동안 예방을 계속해야 해요.\n현지에서 같은 약을 구하기 어려울 수 있으니 6개월분을 함께 가져가세요. 처방전과 제품 포장도 같이 준비하세요.',
        doneSummary: '심장사상충 예방을 했어요.',
        attachmentLabel: '심장사상충 예방',
        validationIds: ['za.heartworm-not-before-infectious-test'],
      },
      // 살진드기·기피 처치 — 바베시아·리슈만편모충 매개체 차단. 출국 30일 이내.
      // ⚠️ '진드기를 죽이는 약'과 '흡혈곤충이 달라붙지 않게 하는 약'은 다르다. 넥스가드
      //   스펙트라처럼 살진드기 효과만 있는 먹는 약으로는 기피 요건을 못 채운다 — 이 구분을
      //   적지 않으면 보호자가 이미 쓰던 약으로 끝났다고 믿는다.
      // ⚠️ 2026-03판 건강증명서에서 처치 항목 구성이 바뀌었다는 정보가 있다. 최종 기준은
      //   발급받은 증명서라, 카드 마지막 줄에서 그 사실을 밝힌다.
      'external-parasite': {
        title: '진드기·흡혈곤충 처치',
        shortLabel: '진드기',
        description:
          '출국 30일 이내에 진드기·흡혈곤충 처치를 하세요.\n\n진드기를 죽이는 약(살진드기제)과 흡혈곤충이 달라붙지 않게 하는 약(기피제)을 모두 써야 해요.\n먹는 진드기약은 죽이는 효과만 있고 기피 효과는 없어요. 퍼메트린 계열 바르는 약이나 목걸이를 함께 쓰는 방법을 수의사와 상의하세요.\n고양이와 함께 지내는 경우 퍼메트린에 노출되지 않도록 주의하세요.\n\n처치 항목은 **발급받은 건강증명서에 적힌 것**을 따르세요.',
        doneSummary: '진드기·흡혈곤충 처치를 했어요.',
        attachmentLabel: '진드기·흡혈곤충 처치',
