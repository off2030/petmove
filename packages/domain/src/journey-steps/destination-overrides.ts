...
> = {
  ...
  turkey: {
    ...
    departure: importQuarantineCard({
      ...
      // 튀르키예는 도착 시 국경 수의검역(서류·마이크로칩·개체 확인)을 한다 — 다른 나라와 같은
      // '검역 받으세요' 패턴. 요건(광견병 항체검사 포함) 충족 시 격리 없이 통과, 미충족 시 격리·
      // 반송. (2026-07-23 재확인. 구 주석 '격리 규정 없음'은 미충족 케이스를 놓친 것. 21일설은
      // 상업 사이트 근거라 카드엔 일수를 못 박지 않고 '격리·반송 가능'으로만 둔다.)
      description:
        '튀르키예 도착 후 공항 동물검역소에서 검역을 받으세요.\n마이크로칩과 서류를 확인해요. 준비에 문제가 없으면 격리 없이 통과할 수 있어요.\n입국 요건을 충족하지 못하면 격리되거나 한국으로 반송될 수 있어요.',
  ...
  israel: {
    ...
    'eu-export-cert': {
      ...
      description:
        ...
        '이스라엘 출국 전 이스라엘 수의당국(Veterinary Services)에서 수출 검역을 받으세요.\n정부 수의사가 발급·인증한 국제건강증명서가 필요하고, 민간 수의사만 서명한 증명서는 인정되지 않아요.\n유효한 광견병 접종을 확인받고, 한국 귀국을 위해 마이크로칩 번호와 광견병 항체검사 결과(0.5 IU/mL 이상, 채혈일·검사기관·결과)가 증명서에 기재돼야 해요.\n검사를 통과하면 국제건강증명서가 발급돼요. 한국 입국 때 이 서류가 반드시 필요해요. (개는 보유 허가증도 함께 준비하세요.)',
