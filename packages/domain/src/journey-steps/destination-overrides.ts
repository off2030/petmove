...
> = {
  ...
  switzerland: {
    ...
    // 귀국 서류 — 이 증명서는 '한국 입국 조건'으로 필수(수출국 정부 인증)이지, 스위스가 출국을
    // 막는 요건이 아니다. 스위스는 EU 비회원국이라 EU 여권 대체 불가(한국 QIA). 필수 서류인데
    // '한국 출국 시 받은 동물검역증으로 대체' 문구는 자기모순이라 제거 — 공통 문구를 안 쓰고
    // 스위스 실제 발급 절차(임상수의사 → 칸톤 수의청 인증)만 남긴다(2026-07-17).
    'eu-export-cert': {
      description:
        '스위스 정부가 인증한 건강증명서를 준비하세요. 한국 입국을 위한 필수 서류예요.\n\n임상 수의사에게 건강증명서를 받아 관할 칸톤(주) 수의청의 공무 수의사에게 인증을 받으세요.',
    },
