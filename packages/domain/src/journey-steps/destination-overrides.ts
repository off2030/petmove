...
> = {
  ...
  // sea-permit 골격에 남아공 고유 절차를 얹는다:
  //   AIA 수입 허가(za-aia-permit, 38) · 검역시설 예약(za-quarantine-reservation, 42) ·
  //   5종 전염병 검사 · 심장사상충 예방 · 살진드기·기피 처치.
  // 호주·뉴질랜드와 형제로 보이지만 **베끼면 틀리는 자리**가 넷이다:
  //   ① **항체 검사(RNATT)가 없다.** 채혈·대기·유효기간 문장을 통째로 가져오면 안 된다.
  //   ② 허가가 **두 장**이다(AIA → 수의검역). AIA 는 개 전용.
