...
> = {
  ...
  hawaii: {
    ...
    // 항공권 — 하와이 입국 제약은 FAVN 검체가 검사기관에 접수된 날로부터 30일(일본 항체 180일
    ...
    departure: importQuarantineCard({
      ...
      description:
        '하와이 도착 후 공항 동물검역소에서 검역을 받으세요.\n마이크로칩과 서류를 확인해요. 준비에 문제가 없으면 공항에서 바로 인계받을 수 있어요(Direct Airport Release).\n입국 요건을 충족하지 못하면 검역소에서 최대 120일간 격리돼요.',
    ...
    // 직전(105)'에서 진드기와 순서 교체. 진드기는 출국 전 14일 이내 창이라 임상검사 쪽이 자연스럽다).
    'us-cdc-dog-import-form': {
      order: 75,
    },
    // 진드기 처치 — 출국 전 14일 이내(long-acting tick product). base 는 일반 문구라 하와이 룰 지목.
    'external-parasite': {
  ...
  // 순서: 항체(40) → 종합백신(50) → **계류시설 예약(95) → 수입 허가(100) → 항공권(102)**.
  ...
  guam: {
    ...
    'flight-purchase': {
      ...
      description:
        '괌 입국 일정에 맞춰 항공권을 구매하세요.\n\n광견병 항체 검사 검체가 검사기관에 접수된 날로부터 120일이 지난 후에 입국하면 계류를 최소화할 수 있어요.\n그 전에도 입국할 수 있지만, 이 경우 남은 기간만큼 괌 지정 시설에서 계류돼요.\n검사 결과는 채혈일로부터 1년간 유효해요.\n항공권 구매 후 계류시설 예약과 수입 허가 신청을 해요.\n예약한 계류시설의 운영시간에 맞는 도착 시간인지 확인하세요.\n항공사에 반려동물 동반 가능 여부를 꼭 확인하세요.',
      descriptionBySpecies: {
        // 생후 6개월은 CDC 규칙이라 **개 전용**(필리핀 4개월령·홍콩 5개월령과 같은 자리).
        dog: '괌 입국 일정에 맞춰 항공권을 구매하세요.\n\n괌 도착일에 생후 6개월 이상이어야 해요.\n광견병 항체 검사 검체가 검사기관에 접수된 날로부터 120일이 지난 후에 입국하면 계류를 최소화할 수 있어요.\n그 전에도 입국할 수 있지만, 이 경우 남은 기간만큼 괌 지정 시설에서 계류돼요.\n검사 결과는 채혈일로부터 1년간 유효해요.\n항공권 구매 후 계류시설 예약과 수입 허가 신청을 해요.\n예약한 계류시설의 운영시간에 맞는 도착 시간인지 확인하세요.\n항공사에 반려동물 동반 가능 여부를 꼭 확인하세요.',
        cat: '괌 입국 일정에 맞춰 항공권을 구매하세요.\n\n광견병 항체 검사 검체가 검사기관에 접수된 날로부터 120일이 지난 후에 입국하면 계류를 최소화할 수 있어요.\n그 전에도 입국할 수 있지만, 이 경우 남은 기간만큼 괌 지정 시설에서 계류돼요.\n검사 결과는 채혈일로부터 1년간 유효해요.\n항공권 구매 후 계류시설 예약과 수입 허가 신청을 해요.\n예약한 계류시설의 운영시간에 맞는 도착 시간인지 확인하세요.\n항공사에 반려동물 동반 가능 여부를 꼭 확인하세요.',
      },
      cardLine: '괌에 입국할 수 있어요.',
      // 계류시설 예약(95) **앞**(2026-07-27 사용자 지적). 시설은 도착 날짜를 잡아 예약하고,
    ...
    departure: importQuarantineCard({
      ...
      //   조건이 기재된다"는 표현이 없다. 계류 기간은 **FAVN 검체가 검사기관에 접수된 날부터
      //   120일 중 남은 기간**으로 계산되고(CQA 예시 포함), 그 내용은 항공권 카드가 말한다.
      //   서류 탭의 수입 허가증 설명에서도 같은 이유로 뺐다.
      description:
        '괌 도착 후 지정 계류시설에서 계류해요.\n\n공항에서 서류와 마이크로칩을 확인한 뒤 계류시설 직원이 동물을 인수해요.\n계류가 끝나면 시설 수의사의 최종 검사를 받고 인도돼요.',
      helpText: '괌 지정 계류시설에 입소한 날짜',
  ...
  canada: {
    ...
    // CBSA 국경 심사는 '수입 검역'이 아니라 미국 CBP 와 같은 '입국 검사' 그룹 —
    // 헬퍼 기본 제목(수입 검역)을 입국 검사 규격으로 덮는다.
    departure: {
  ...
  usa: {
    ...
    'flight-purchase': {
      ...
      // 생후 6개월은 CDC 개 수입 규칙이라 **개 전용** — 통합문만 두면 고양이 카드에
      //   '강아지는…' 줄이 그대로 떴다(괌 항공권 카드와 같은 분리, 2026-08-01).
      //   description(종 미상 폴백)은 위 통합문 유지.
      descriptionBySpecies: {
        dog: '미국 입국 일정에 맞춰 항공권을 구매하세요.\n\n미국 도착일에 생후 6개월 이상이어야 해요.\n항공사에 반려동물 동반 가능 여부를 꼭 확인하세요.',
        cat: '미국 입국 일정에 맞춰 항공권을 구매하세요.\n\n항공사에 반려동물 동반 가능 여부를 꼭 확인하세요.',
      },
      cardLine: '미국에 입국할 수 있어요.',
      earliest: undefined,
  ...
  // ── 인도 — 종합백신 종별 분리만 ──────────────────────────────────────
  // 종합백신 카드국 중 인도만 override 가 없어 base 통합문('강아지는 DHPP(C), 고양이는
  //   FVRCP를…')이 고양이 카드에도 '강아지는…' 채로 노출됐다(2026-08-01 종별 분리).
  //   인도 규정 세부는 조사된 바 없어 base 내용을 종별로 나누는 수준만 — 새 규정 문구 금지.
  india: {
    'general-vaccine': {
      // 종 미상 폴백 — base 통합문 그대로(descriptionBySpecies 를 쓰는 카드는 description 도
      //   항상 채운다 — types.ts 주석).
      description:
        '강아지는 DHPP(C), 고양이는 FVRCP를 접종하세요. 출국 시점에 유효기간이 남아있어야 해요.',
      descriptionBySpecies: {
        dog: '종합백신(DHPP(C))을 접종하세요.\n\n출국 시점에 유효기간이 남아있어야 해요.',
        cat: '종합백신(FVRCP)을 접종하세요.\n\n출국 시점에 유효기간이 남아있어야 해요.',
      },
    },
  },
  ...
  taiwan: {
    ...
    'flight-purchase': {
      description:
        // 격리 면제 조건(120일 전 신청 or 직송) 안내는 수입허가증 카드(order 43)가 담당 — 중복이라 뺀다.
      ...
      // 수입허가 마감(도착 20일 전) 주의도 여기에 둔다 — 수입허가 카드가 아니라.
      ...
      validationIds: [
        ...
        'tw.import-permit-20days-before-entry',
        // 접종 후 선적 대기(1차 90일/부스터 30일)도 여기 — 접종일은 과거 사실이라
        // 어긋나면 바꿀 수 있는 건 출국일뿐이다.
        'tw.rabies-shipment-window',
      ],
    },
    // 수입허가증 — APHIA pet e-permit 온라인 신청. 마감 = 도착 20일 전 단일.
    //
    // 격리 면제는 신청 시점만으로 정해지지 않는다(문답집 1150310 확인, 2026-08-04): 채혈 후
    // 180일 조건에 더해 ①輸入 120일 전 신청 또는 ②20일 전 신청 + 검사기관(실험실)·수출국
    // 검역기관이 rabiesreport@aphia.gov.tw 로 보고서 직송(또는 실험실 웹사이트 조회 — 한국
    // 중앙백신(CAvac) 해당). 로잔 대행 실무는 ② 경로가 기본이라 120일 배지·주의는 제거하고
    // 면제 조건은 아래 설명문이 말한다(사용자 확정 문구).
    //
    // 항공권(45) 앞에 둔다(base 100 → 43). 대만은 신청에 항공권이 필요 없고(예정 입국일·도착
    // 공항만, 이후 1회 변경 가능) — 실무에서도 허가 신청을 먼저 한다. 태국·필리핀(90·95 → 100)은
    // 반대로 신청에 항공편 일정이 필수라 항공권이 앞이다.
    // 두 절차 사이에 의존이 없을 뿐 '동시에 하는 일'은 아니므로 concurrent 는 쓰지 않는다.
    'import-permit': {
      order: 43,
      description:
        '대만 수입 허가를 신청하세요.\n\n도착 20일 전까지 온라인으로 신청하세요.\n\n격리하지 않으려면 120일 전까지 수입 허가를 신청하거나, 검사기관이 대만 담당기관으로 직접 결과를 전달해야 해요.',
      doneSummary: '대만 수입 허가증을 받았어요.',
      cardLine: '대만 수입 허가를 신청하세요.',
      deadline: { anchor: 'departure', daysBefore: 20 },
      inputs: [{ key: 'import_permit_application_date', label: '신청일', type: 'date' }],
      attachmentHint: '수입 허가증(Import Permit)을 사진·PDF로 보관하세요.',
      attachmentLabel: '대만 수입 허가증',
      links: [
  ...
  australia: seaPermitOverrides({
    ...
    //   카드에 넣지 않은 인수 조건(오전 10~12시 · 사진 신분증 · 주소 135 Donnybrook Rd
    //     Mickleham · 이동장 회수 의무 · 석방일 연장 불가)은 필요해지면 서류 탭이나 별도 안내로.
    //     마지막 줄은 "찾아갈 수 있다"는 결말만 담는다 — 인수 주체(허가서상 수입자 또는 사전
    //     통보한 대리인)만 원문에서 가져왔다.
    importQuarantine: {
    ...
    extra: {
      ...
      'rabies-titer': {
        description:
          '호주 검역당국(DAFF)이 인정하는 검사기관에서 광견병 항체 검사를 받으세요.\n\n동물병원을 통해 의뢰할 수 있어요.\n0.5 IU/mL 이상이면 합격이에요.\n검사 결과는 채혈일로부터 1년간 유효해요.',
      ...
      'kennel-cough-vaccine': {
        order: 36,
        // 유효기간 줄은 2026-07-30 사용자 지정으로 추가 — 완료 판정(has-kennel-cough)이
        //   유효기간을 보는데 카드가 그 사실을 말하지 않아, 접종일을 넣어도 왜 미완료인지
        //   알 수 없었다. 다른 백신 카드와 같은 문장으로 통일했다.
        description:
          '켄넬코프(Bordetella) 백신을 접종하세요.\n\n호주 입국 요건은 아니지만, 계류시설 예약을 위해 필요해요.\n출국 때 면역 유효기간이 남아있어야 해요.',
      },
      // 전염병 검사 — 리슈만편모충(전 개체) + 브루셀라(중성화 안 한 개체) + 렙토 MAT(백신 미접종 시).
      //   세 검사 모두 출국 전 45일 이내 채혈이라 한 카드에 묶는다(admin 도 같은 필드를 공유).
      ...
      // 문구는 사용자 확정본 3줄 그대로(2026-07-28). ⛔ 다듬거나 되돌리지 말 것 —
      //   원문에 있는 세부(접촉 살충·병원 방문마다 검사·발견 시 재시작·못 쓰는 약 목록)를
      //   일부러 덜어낸 판단이다. 종별 일수만 갈린다(개 30일 / 고양이 21일 — DAFF 7.6 / 고양이
      //   가이드 7.3, 2026-07-28 원문 재확인).
      'external-parasite': {
        // 운송 예약(46) 뒤로 **전염병 검사(70)·외부구충(80)·내부구충(90) 세 장을 '다음 할 일'에
        //   함께** 띄운다(2026-07-30 사용자 지정). 전염병 검사는 출국 직전에 결과가 나오고
        //   그 대기 중간에 구충을 해야 하는데, 순차로 뜨면 전염병 검사 하나에 막혀 구충이
        //   화면에서 사라진다(항체 검사 진행 중과 같은 형태).
        // 승격 로직이 mainIdx **직후로 이어지는** concurrent 를 올리므로, 전염병 검사가
        //   mainIdx 가 되고 외부(80)·내부(90, base concurrent)가 함께 올라온다. 그래서
        //   플래그는 전염병 검사가 아니라 **외부구충**에 붙인다.
        // ⚠️ 호주 전용 — base 에 붙이면 카드가 하나뿐인 7개국까지 끌려간다. 뉴질랜드는
        //   1차 외부구충이 전염병 검사보다 **앞**(order 68)이고 채혈 14일 전 의존이 있어
        //   같은 묶음이 성립하지 않는다(별도 처리 예정).
        concurrent: true,
        description:
          '외부 기생충 치료를 하세요.\n\n강아지는 출국 30일 전, 고양이는 21일 전에 시작해서 약효가 끊기지 않도록 반복해요.\n호주에서 인정하는 성분을 함유한 제품을 사용해야 해요.',
        descriptionBySpecies: {
      ...
      // 내부구충 — 45일 이내 2회 · 14일 이상 간격 · 2차는 출국 전 5일 이내(7.7).
      ...
      'internal-parasite': {
        description:
          '내부 기생충 치료를 하세요.\n\n출국 전 45일 이내에 14일 이상의 간격으로 2회 이상 치료해요.\n2차 치료는 출국 전 5일 이내에 해야 해요.\n호주에서 인정하는 성분을 함유한 제품을 사용해야 해요.',
        links: [
          {
            url: 'https://www.agriculture.gov.au/biosecurity-trade/cats-dogs/step-by-step-guides/parasite-treatment',
            label: '인정 성분·제품 목록(DAFF)',
      ...
      //   · 마지막 줄이 다른 목적지의 공용 문형('이 서류를 발급하지 않는 동물병원도 있으니
      //     미리 확인하세요.')과 다르다 — 전염병 검사 카드와 같은 문형으로 맞춘 사용자
      //     판단이다. 공용 문형으로 되돌리지 말 것.
      //   ⚠️ 이 문형은 **목적지 전용 서식을 함께 발급받는 곳**의 문형이다(2026-07-30 사용자
      //     지정): 호주·뉴질랜드·일본(FormAC)·대만(APHIA 002)·싱가포르(Schedule III)·
      //     EU(Annex III). 별지 제25호만 받는 나머지 31곳은 구 공용 문형('…동물병원도 있으니
  ...
  new_zealand: seaPermitOverrides({
    ...
    generalVaccine: {
      ...
      order: 32,
      // 고양이 케이스에 **개 백신 이름**(디스템퍼·파보…)이 그대로 떠 있었다(2026-07-30 발견).
      //   종별 본문을 나눈다 — 괌·UAE 와 같은 처리. 종 미확정일 때 쓰는 기본 문구는 둘을
      //   한 줄에 담는다(괌 패턴).
      description:
        '종합백신을 접종하세요.\n\n강아지는 디스템퍼·전염성간염·파보바이러스·파라인플루엔자·렙토스피라, 고양이는 범백혈구감소증·허피스바이러스·칼리시바이러스 예방을 포함해야 해요.\n뉴질랜드 입국 요건은 아니지만, 계류시설 예약을 위해 필요해요.',
      descriptionBySpecies: {
        dog: '종합백신을 접종하세요.\n\n디스템퍼·전염성간염·파보바이러스·파라인플루엔자·렙토스피라 예방을 포함해야 해요.\n뉴질랜드 입국 요건은 아니지만, 계류시설 예약을 위해 필요해요.',
        cat: '종합백신을 접종하세요.\n\n범백혈구감소증·허피스바이러스·칼리시바이러스 예방을 포함해야 해요.\n뉴질랜드 입국 요건은 아니지만, 계류시설 예약을 위해 필요해요.',
      },
      // 시점 룰(‘출국 N일 전’·‘12개월 이내’)은 두지 않는다 — 입국 요건이 아니라 시설
      //   요구사항이라 규정상 기한이 없다. 만료 주의(common.general-vaccine-validity-expired)는
      //   **base 카드가 이미 달고 있어** 여기서 다시 적지 않는다 — 적으면 같은 룰이 두 번
      //   들어가 주의가 중복으로 뜬다(2026-07-30 발견, 뉴질랜드만 그랬다).
    },
    // 카드 이름은 **호주와 동일하게 '운송 예약'**(2026-07-29 사용자 확정). 지원문서가 '정식
    //   항공 화물 전용'이라고 못박지는 않지만(crate in the cargo hold), 실무가 호주와 같아
    //   보호자가 두 나라를 오갈 때 같은 이름으로 읽히게 맞춘다.
    ...
    flight: {
      ...
      attachmentHint: '운송 예약 확인서·항공권(e-티켓)을 사진·PDF로 보관하세요.',
      description:
        // 채혈 창을 **마이크로칩 인증 횟수로 갈라** 설명한다(2026-07-29 사용자 지정). 규정은
        //   채혈이 출국 3~12개월 전이고, 그중 3~6개월 구간은 인증을 2회 받아야 한다(IHS 1.11(4)).
        //   출국일을 잡는 카드에서는 "인증을 몇 번 받았나 → 언제 나갈 수 있나"로 읽히는 게 맞다.
        '뉴질랜드 입국 일정에 맞춰 운송을 예약하세요.\n\n오클랜드나 크라이스트처치 공항으로만 도착할 수 있어요.\n마이크로칩 인증을 1회 받았다면 채혈 후 6~12개월 사이에 출국해야 해요.\n2회 받았다면 채혈 후 3~12개월 사이에 출국해야 해요.',
    ...
    // 도착 = 오클랜드·크라이스트처치 공항 → MPI 승인 계류시설로 직행(IHS 1.15(3)), 최소 10일.
    //   ⛔ 호주의 '마이크로칩 인증을 받았으면 10일, 아니면 30일' 문장을 복사하지 말 것 —
    //     뉴질랜드는 인증이 필수라 분기가 없다.
    //   문구는 사용자 확정본 4줄(2026-07-30, 호주 카드 구성 참고 후 직접 지정).
    //     ⛔ 되돌리지 말 것 — 사실이지만 일부러 뺀 세 줄이 있다:
    //       · '서류·검사·기생충에 문제가 있으면 계류가 길어질 수 있어요.'(호주에서도 같은 이유로 뺐다)
    //       · '이동은 시설이 준비해요.' / 수입 허가에 적힌 시설이라는 단서(IHS 1.15(3))
    ...
    extra: {
      ...
      //   브루셀라(2.8, 중성화 안 한 개체). **네 검사 모두 출국 전 30일 이내 채혈**이라 한 카드에 묶는다.
      ...
      //   (2.12) / 브루셀라: RSAT·TAT(2.8) / 렙토: MAT(2.13).
      // ⛔ 렙토에 **호주 문장을 가져오지 말 것**(2026-07-29 사용자 지적으로 정정). 호주는 렙토
      //   Canicola 백신을 맞으면 MAT 를 면제하지만, 뉴질랜드 2.13 의 선택지는 ①수출국이 청정국
      //   ②MAT 음성 ③독시사이클린 14일 투약 셋뿐이고 **백신은 면제 사유가 아니다**. 백신이
      //   등장하는 자리는 MAT 가 약한 양성(1:100~1:400)일 때 그 수치를 설명하는 근거뿐이다.
      // 규정 전수 대조(2026-07-29) 후 보강한 네 가지:
      //   · 바베시아 — 생후 6개월 미만은 IFAT/ELISA 가 인정되지 않고 PCR 3회만(2.7(2)).
      //   · 리슈만 — 혈청 검사를 방해하는 백신을 맞았으면 IFAT/ELISA 가 아니라 buffy coat·
      //     결막 PCR 이다(2.12). '"IFAT 또는 ELISA만"' 으로 적으면 그 개가 틀린 검사를 받는다.
      //   · 브루셀라 — 중성화견도 할 일이 있다(수의사 서명 중성화 증명서, 2.8).
      //   · 렙토 — 검사 대신 독시사이클린 14일 투약도 인정된다(2.13). MPI 는 항생제 남용을
      //     줄이려 검사를 우선 권하지만, 검사가 어려운 케이스의 유일한 우회로다.
      // ⚠️ 남은 간극 — 바베시아 PCR 3회 옵션은 채혈 창이 **44일**인데(2.7) 리드 문장과 주의
      //   룰(nz.infectious-disease-test-within-30days)은 30일로 본다. 생후 6개월 미만 케이스가
      //   40일 전에 1차 PCR 을 받으면 주의가 잘못 뜬다. 회차를 구분해 저장하지 않아 지금은
      ...
      //   1차 = 바베시아 채혈 14일 전까지 / 2차(마지막) = 출국 전 16일 이내 / 1차부터 출국까지 연속 보호.
      //   ⛔ 호주의 '출국 30일 전에 시작' 문장을 가져오지 말 것 — 뉴질랜드 하한은 바베시아 채혈 기준이다.
      'external-parasite': {
        // 전염병 검사(70) **앞**에 둔다(2026-07-29 사용자 지정). 뉴질랜드는 1차 외부구충 14일
        //   뒤에야 바베시아 채혈이 가능해 순서가 규정으로 정해져 있는데, base 순서(검사 70 →
        //   외부구충 80)는 그 반대라 카드가 할 수 없는 일을 먼저 시켰다. 호주는 바베시아가
        //   없어 이 의존이 없으므로 base 순서 그대로 둔다.
        order: 68,
        // ⛔ 되살리지 말 것(2026-07-29 사용자 지정으로 삭제) —
        //   · '진드기와 벼룩을 없애는 제품으로 두 번 치료해요'(2.2(2) 회수·대상 기생충)
        //   회수는 아래 1차·2차 두 줄이 이미 드러내고, 제품 요건은 승인 목록이 담당한다.
        // 검진 확인 줄은 **되살렸다**(2026-07-30 사용자 '맞추자') — 2.2(2)(d) 는 개에게도
        //   매 처치 시점과 출국 2일 전 검진에서 외부 기생충이 없을 것을 요구한다. 고양이
        //   문구(2.2(1)(c))에만 남아 두 종이 다른 안내를 받고 있었다. 문형은 고양이와 같게.
        // 1차→2차 상한(구 '28일 이내')은 삭제 — 규정에 없는 근사치였고 장기 지속 제품
        //   (브라벡토 12주 등)의 정상 일정을 막았다(date-rules 의 ⛔ 주석 참고).
        ...
        yieldsWhenInProgress: true,
        description:
          '외부 기생충 치료를 하세요.\n\n바베시아 검사 채혈 14일 전에 시작해서 약효가 끊기지 않도록 반복해요.\n마지막 치료는 출국 전 16일 이내에 해요.\n치료할 때와 출국 2일 전 검진에서 진드기·벼룩이 없는지 확인받아요.\n뉴질랜드 검역당국(MPI)이 승인한 제품을 사용해야 해요.',
        // 문형은 **호주와 같게**(2026-07-30 사용자 지정) — 시작 하한과 연속 보호를 한 줄로
        //   묶고('…에 시작해서 약효가 끊기지 않도록 반복해요'), 회차는 '1차/2차' 대신
        //   '시작/마지막'으로 부른다. 내부구충은 이미 호주와 같은 문형이라 그대로 둔다.
        // 고양이는 회수·기준이 다르다 — IHS 2.2(1): **한 번**, 출국 전 16일 이내, 그때부터
        //   출국일까지 연속 보호. 바베시아 검사가 개 전용이라 '채혈 14일 전' 하한이 없다.
        //   개 문구를 그대로 보여주면 없는 절차를 요구하게 된다.
        descriptionBySpecies: {
          dog: '외부 기생충 치료를 하세요.\n\n바베시아 검사 채혈 14일 전에 시작해서 약효가 끊기지 않도록 반복해요.\n마지막 치료는 출국 전 16일 이내에 해요.\n치료할 때와 출국 2일 전 검진에서 진드기·벼룩이 없는지 확인받아요.\n뉴질랜드 검역당국(MPI)이 승인한 제품을 사용해야 해요.',
          // 사용자 확정본 2줄(2026-07-30). ⛔ 다듬지 말 것 — 사실이지만 뺀 두 줄이 있다:
          //   '출국일까지 약효가 끊기지 않도록 반복해요'(2.2(1)(b))
          //   '치료할 때와 출국 2일 전 검진에서 진드기·벼룩이 없는지 확인받아요'(2.2(1)(c))
          cat: '출국 전 16일 이내에 외부 기생충 치료를 하세요.\n\n뉴질랜드 검역당국(MPI)이 승인한 제품을 사용해야 해요.',
        },
      ...
      // 내부구충 — IHS 2.3(1): 1차 출국 전 30일 이내 + 14일 이상 간격 + 2차 출국 전 5일 이내.
      ...
      // 문형은 **호주와 같게**(2026-07-29 사용자 지정) — 창·간격·회수를 한 줄로 합치고 2차만
      ...
      'internal-parasite': {
        // base 의 concurrent 를 **끈다** — 뉴질랜드는 외부구충 1차가 바베시아 채혈 14일 전이라
        //   내부구충(출국 전 30일 이내)과 **같은 날이 아니다**(2.2(2)(a)). 다른 나라의 '내·외부는
        ...
        yieldsWhenInProgress: true,
        description:
          '내부 기생충 치료를 하세요.\n\n출국 전 30일 이내에 14일 이상의 간격으로 2회 치료해요.\n2차 치료는 출국 전 5일 이내에 해야 해요.\n뉴질랜드 검역당국(MPI)이 승인한 제품을 사용해야 해요.',
        links: [
          {
            url: 'https://www.mpi.govt.nz/dmsdocument/2040-Specified-Approvals-for-Animal-Import-Health-Standards-MPI-STD-SAA',
            label: '승인 제품 목록(MPI-STD-SAA, PDF)',
      ...
      //   출국 전 5일 이내라 같은 방문에서 끝나지만 약이 달라 별도 카드로 둔다.
      'lungworm-treatment': {
        description:
      ...
      // 심장사상충 — IHS 2.11. 검사(생후 7개월 이상, 출국 전 30일 이내)와 예방 투약(출국 전 5일 이내)이
      //   한 카드에 들어간다. 카드 입력칸은 날짜 배열 하나(heartworm_dates)라 두 사실을 같이 적는다.
      // 심장사상충 — IHS 2.11 은 **검사(출국 전 30일 이내)와 예방 투약(5일 이내)이 시기가 다른
      //   별개 항목**이다. 검사는 시기가 전염병 검사와 완전히 같아(둘 다 출국 전 30일 이내) 그
      ...
      'heartworm-test': {
        ...
        // 출국 전 5일 이내(2.11(2)(a)) — 주의 + **저장 거부**(validateNzWithin5Days) 둘 다.
  ...
  // ── 남아프리카공화국 (DALRRD — 강아지) ─────────────────────────────────────
  // 출처: 남아공 정부 동물 수입 안내(gov.za) · 농업부 Animal Health 신청서·수수료 문서 ·
  ...
  //   ① **항체 검사(RNATT)가 없다.** 채혈·대기·유효기간 문장을 통째로 가져오면 안 된다.
  //   ② 허가가 **두 장**이다(AIA → 수의검역). AIA 는 개 전용이고 순서가 규정 명문이다.
  //   ③ 계류가 약 14일이고 **한국이 면제국이 아니라서** 붙는다 — 인증·품종 분기가 없다.
  //   ④ 심장사상충은 **도착 후 6개월까지** 계속한다(출국으로 끝나지 않는다).
  //   ⑤ 검역시설 예약이 수입 허가 **뒤**다. 뉴질랜드는 예약 확인서가 허가 신청 제출물이라
  //      예약(42) → 허가(44) 인데, 남아공은 검역 대상 여부가 허가서에 명시되고 그걸 보고
  //      예약한다 — 순서가 반대다(각 카드 ⛔ 주석 참고).
  south_africa: seaPermitOverrides({
    key: 'south_africa',
    label: '남아프리카공화국',
    // 30일 대기·12개월 유효는 두 조건이 한 문장에 붙어 있다. 유효한 접종 안에서 추가접종하면
    //   30일 대기가 면제되는 것도 규정에 명시돼 있어 카드에 적는다.
    // 사용자 확정본 5줄(2026-07-30 직접 지정). ⛔ 다듬지 말 것 — 사실이지만 **일부러 뺀** 줄이 셋이다:
    //   · '마이크로칩 삽입 후에 접종해야 해요.' — 근거 자체가 없어 삭제했다. HA2123 증명서 A.1 은
    //     칩 번호·위치 표와 "Microchip must be able to be read by ISO 11784 or ISO 11785
    //     scanners" 한 줄이 전부고, 접종·채혈과의 **순서 조항이 없다**. EU(Reg 576/2013)와 다르다.
    //   · '이전 접종의 유효기간 안에 추가접종하면 30일을 기다리지 않아도 돼요.' — 각주 1) c) 의
    //     실제 면제 조항이고 **판정도 그대로 면제한다**(violatesRabiesEntryWait). 문구에서만 빼서
    //     예외를 앞세우지 않기로 한 것 — 룰을 30일 무조건으로 바꾸지 말 것.
    //   · '남아프리카공화국 입국에는 광견병 항체 검사가 필요하지 않아요.' — 요건이 아니라 부가 안내.
    // '출국 때 면역 유효기간' — 각주 1) b) 가 prior to **export** 라 입국이 아니라 출국이 맞다.
    rabiesDescription:
      '광견병 백신을 접종하세요.\n\n생후 3개월이 지난 후에 접종해야 해요.\n출국 30일 전까지 접종해야 해요.\n면역 유효기간은 1년이에요. 2년·3년 백신은 인정되지 않아요.\n출국 때 면역 유효기간이 남아있어야 해요.',
    // 30일 대기·출국일 유효기간은 **운송 예약 카드**가 받는다(아래 flight.validationIds) —
    //   접종일은 지나간 사실이라 이 카드에서 할 수 있는 조치가 없고, 고칠 수 있는 건 출국일이다
    //   (베트남과 같은 배치). 여기 남는 건 접종 기록 자체의 요건이다.
    rabiesValidationIds: [
      'za.rabies-prime-after-3months-old',
      'za.rabies-booster-within-prime-validity',
      // blocker(저장 거부)인데 카드에 안 붙으면 경고가 상단 caseAlert 로 샌다 — 1회 접종국이라
      //   2차 카드가 없으므로 이 카드가 받는다(베트남과 같은 처리).
      'za.rabies-only-1year-vaccine',
    ],
    // ⚠️ titerDescription 을 넘기지 않는다 — 남아공은 입국에도 귀국에도(편도 전용) 항체
    //   검사가 없다. 프로파일 titer.need='none' 과 짝이다. 홍콩과 같은 처리.
    // ⚠️ generalVaccine 도 넘기지 않는다 — 종합백신은 남아공 입국 요건이 아니고
    //   GENERAL_VACCINE_CARD_DESTINATIONS 에도 넣지 않았다(근거 없이 넣지 않는다 —
    //   튀르키예·인도네시아와 같은 판단).
    // 화물(manifest cargo) 전용 — 보호자와 같은 항공편이어도 반려동물은 화물로 등록하고
    //   AWB 를 받는다. 그래서 카드 이름이 '항공권 구매'가 아니라 **'운송 예약'**이다
    //   (호주·뉴질랜드·홍콩과 같은 이름).
    flight: {
      title: '운송 예약',
      shortLabel: '운송',
      ...
      attachmentHint: '운송 예약 확인서·항공권(e-티켓)을 사진·PDF로 보관하세요.',
      description:
        // 대기 요건이 광견병 30일 하나뿐이라 문장이 짧다 — 호주·뉴질랜드의 '채혈 후 N개월'
        //   줄을 가져오지 말 것. 대신 남아공에만 있는 두 제약을 적는다: 화물 운송, 도착 공항.
        //   검역 대상 강아지는 검역시설이 있는 두 공항으로만 들어갈 수 있다.
        // 사용자 확정본 3줄(2026-07-30 직접 지정). 화물 줄만 홍콩·영국형 문장 구조로
        //   재정렬(2026-08-01 문구 통일 — Manifest Cargo·AWB 용어는 유지). ⛔ 뺀 줄 넷은 되살리지 말 것:
        ...
        '남아프리카공화국 입국 일정에 맞춰 운송을 예약하세요.\n\n기내 반입이나 위탁수하물로는 갈 수 없어요. 항공화물(Manifest Cargo)로 보내야 하므로 동물 운송업체와 미리 협의하세요. 예약 후 화물운송장(AWB)을 받아요.\n강아지는 요하네스버그(JNB)나 케이프타운(CPT) 공항으로만 도착할 수 있어요. 두 곳에만 계류시설이 있어요.',
      ...
      descriptionBySpecies: {
        dog: '남아프리카공화국 입국 일정에 맞춰 운송을 예약하세요.\n\n기내 반입이나 위탁수하물로는 갈 수 없어요. 항공화물(Manifest Cargo)로 보내야 하므로 동물 운송업체와 미리 협의하세요. 예약 후 화물운송장(AWB)을 받아요.\n요하네스버그(JNB)나 케이프타운(CPT) 공항으로만 도착할 수 있어요. 두 곳에만 계류시설이 있어요.',
      ...
      validationIds: [
        ...
        'za.rabies-valid-on-departure',
        // 출국일 ≤ 검역 시작일 — 조치 자리가 출국일 쪽이라 여기(호주·뉴질랜드·싱가포르와 동일).
        'za.quarantine-start-not-before-departure',
      ],
    },
    // 수의검역 수입 허가 — Director: Animal Health(VetPermits) 이메일 신청.
    //   선행은 **AIA 허가 하나뿐**이다(규정 명문 — importPermitPrerequisiteError 가 막는다).
    //   ⛔ '검역시설 예약을 먼저 끝내야'로 되돌리지 말 것(2026-07-30 사용자 확정으로 정정).
    //     검역 대상 여부가 **허가서에 명시되고** 그걸 보고 예약한 뒤 예약 확인서를 신청 과정에
    //     반영하는 흐름이다(주신 자료: "수입허가서에 검역이 명시되면 → State Vet 연락 → 자리
    //     예약 → 예약 확인을 수입허가 신청서에 반영"). 초안의 "검역 자리가 확보되어야 수입허가
    //     절차를 **마칠** 수 있으므로"도 신청이 아니라 **발급**을 가리킨다. 그래서 순서는
    //     허가(40) → 예약(42) 이고, 앱의 신청→발급 2단계 카드가 그 구간을 함께 덮는다.
    ...
    importPermit: {
      ...
      doneSummary: '남아프리카공화국 수입 허가증을 받았어요.',
    ...
    importQuarantine: {
      fieldKey: 'za_import_quarantine_date',
      description:
        '남아프리카공화국 도착 후 화물터미널에서 서류와 마이크로칩을 확인해요.\n\n강아지는 확인이 끝나면 국가 계류시설로 옮겨져요.\n계류 기간은 약 14일이에요.\n계류 중에 5가지 전염병 검사를 다시 할 수 있어요.\n검사와 건강 상태에 문제가 없으면 보호자나 대리인이 데려갈 수 있어요.',
      // 고양이는 검역 없이 화물터미널에서 통관·인도된다 — 개와 절차가 통째로 다르다.
      descriptionBySpecies: {
        dog: '남아프리카공화국 도착 후 화물터미널에서 서류와 마이크로칩을 확인해요.\n\n확인이 끝나면 국가 계류시설로 옮겨져요.\n계류 기간은 약 14일이에요.\n계류 중에 5가지 전염병 검사를 다시 할 수 있어요.\n검사와 건강 상태에 문제가 없으면 보호자나 대리인이 데려갈 수 있어요.',
        cat: '남아프리카공화국 도착 후 화물터미널에서 수입 검역과 통관을 받아요.\n\n원본 서류와 마이크로칩에 문제가 없으면 대체로 당일에 데려갈 수 있어요.\n여객 수하물 구역이 아니라 화물터미널에서 진행되니 통관 가능 시간을 미리 확인하세요.',
      },
      helpText: '남아프리카공화국 도착 후 계류를 시작한 날짜',
      attachmentHint: '검역 서류 사본을 사진·PDF로 보관하세요.',
      attachmentLabel: '남아프리카공화국 수입 검역 서류',
      validationIds: ['za.import-quarantine-date-valid'],
      infoOnly: true,
    },
    extra: {
      // 5종 전염병 검사 — 채혈일부터 **남아공 도착일**까지 30일. 앱은 출국일을 기준으로 보되
      //   창을 29일로 잡아 비행 하루치 여유를 둔다(za.ts 헤더 참고).
      // ⚠️ 검사법을 반드시 함께 적는다 — 남아공은 병명이 아니라 **검사법**을 지정한다.
      //   심장사상충은 항원 검사(SNAP 등)가 아니라 microfilarial filtration test 라, 국내에서
      //   흔히 하는 검사를 그대로 받으면 인정되지 않는다. 이 한 줄이 없으면 헛검사를 한다.
      'infectious-disease-test': {
        // 사용자 확정본 8줄(2026-07-30 직접 지정). ⛔ 다듬지 말 것 — 뺀 줄이 넷이다:
        //   · '검사 결과지는 영문으로 발급받고 마이크로칩 번호가 적혀 있어야 해요.'
        //   · '남아공이 인정하는 검사기관에서 받아야 하며, 국내에서 어려운 검사는 해외 의뢰가
        //     필요할 수 있어요.' → '모든 검사는 남아프리카공화국 지정 검사기관에서 받아야 해요.'로 교체
        //   · '검사법은 **발급받은 건강증명서에 적힌 것**을 따르세요. 허가가 나오기 전에 검사부터
        //     시작하면 검사법이 맞지 않을 수 있어요.'
        //   · '남아공 도착 후 검역 중에 같은 검사를 다시 할 수 있고, 비용은 보호자가 부담해요.'
        // 검사법 5줄은 HA2123 증명서 5.1~5.5 원문 대조본 그대로 유지(2026-07-30). 바베시아는
        ...
        //   임상검사'(110, 출국 전 10일 이내)가 한 달도 더 전에 '다음 할 일'로 떴다.
        validationIds: ['za.infectious-disease-within-29days'],
      },
      // 심장사상충 — 검사(5종에 포함)와 **예방 투약**은 별개다. 이 카드는 예방만 담당한다
      //   (뉴질랜드와 같은 분리). 남아공만의 특이점은 **도착 후 6개월까지** 계속한다는 것.
      // 사용자 확정본 3줄(2026-07-31 직접 지정 — 2차). ⛔ 다듬지 말 것 — 뺀 줄이 다섯이다:
      //   · '1년 지속형 주사(ProHeart SR12)를 쓴 경우, 채혈일이 주사 후 11개월 이내면 추가
      //     투약이 필요 없어요.'(각주 2) 근거는 있다)
      //   · '약 이름·용량·투여일을 기록해두세요. 건강증명서에 적어야 해요.'
      //   · '처방전과 제품 포장도 같이 준비하세요.'
      //   · '디에틸카바마진·이버멕틴·밀베마이신 옥심·목시덱틴·셀라멕틴 중 증명서가 허용한
      //     약을 써요.'(6.1 원문 목록이지만 뺐다)
      //   · '현지에서 같은 약을 구하기 어려울 수 있으니 6개월분을 함께 가져가세요.'
      //     → '예방약은 한국에서 준비해가야 해요.'로 3번째 줄에 합쳤다.
      'heartworm-test': {
        title: '심장사상충 예방',
        // 전염병 검사(70) 바로 뒤 — 예방 시작일이 **그 검사의 채혈일**이라 같은 방문에서 끝난다
        //   (2026-07-31 사용자 지정). base 108 은 검사·예방을 한 카드로 보던 시절 자리였다.
        order: 72,
        description:
          '심장사상충 예방을 시작하세요.\n\n심장사상충 검사일로부터 출국일까지 끊기지 않게 투약해요.\n남아프리카공화국에 도착한 뒤에도 6개월 동안 예방을 계속해야 해요. 예방약은 한국에서 준비해가야 해요.',
        // ⚠️ 입력 라벨('심장사상충 예방'/'투약일'/'+ 투약 기록 추가')은 여기서 못 바꾼다 —
        //   date_array 위젯은 step.inputs 의 label 을 읽지 않고 step-detail-view 의
        //   isHeartwormPreventionOnly 분기가 정한다. 거기에 south_africa 를 넣어 뒀다.
        doneSummary: '심장사상충 예방을 했어요.',
        attachmentLabel: '심장사상충 예방',
        validationIds: ['za.heartworm-same-day-as-infectious-test'],
      },
      // 외부 기생충 치료 — 바베시아·리슈만편모충 매개체 차단(살진드기제 + 기피제). 출국 전 30일 이내.
      // 사용자 확정본 2줄(2026-07-31 직접 지정). ⛔ 다듬지 말 것 — 뺀 줄이 넷이다:
      //   · '진드기를 죽이는 약(살진드기제)과 흡혈곤충이 달라붙지 않게 하는 약(기피제)을 모두 써야 해요.'
      //   · '먹는 진드기약은 죽이는 효과만 있고 기피 효과는 없어요. 퍼메트린 계열 …'
      //   · '고양이와 함께 지내는 경우 퍼메트린에 노출되지 않도록 주의하세요.'
      //   · '처치 항목은 발급받은 건강증명서에 적힌 것을 따르세요.'
      //   ⚠️ 뒤 셋은 **규정이 아니라 내가 합성한 지침**이었다 — 증명서 6.2 는 "an effective
      //     acaricide and with insect repellent **registered in the exporting country**" 뿐이고,
      //     남아공이 승인 제품 목록을 내지 않는다(기준이 한국 등록 제품이라 걸 링크가 없다).
      //     실제로 프론트라인 플러스로 통과한 사례가 있어(2026-07-31 사용자 확인) 없는 제약을
      //     안내하고 있었다. ⛔ 퍼메트린 권고를 되살리지 말 것.
      //   제목·완료문·첨부명은 **base 그대로** 쓴다(구 '진드기·흡혈곤충 처치' override 제거).
      'external-parasite': {
        // 심장사상충 예방(72) 뒤 — 전염병 검사와 같은 방문에서 함께 끝나는 묶음(2026-07-31).
        order: 74,
        // 전염병 검사(70)가 '다음 할 일'이 되면 심장사상충(72)·이 카드까지 **함께** 뜬다
        //   (scenario.ts head run — mainIdx 뒤로 이어지는 concurrent 를 모두 올린다).
        concurrent: true,
        description: '외부 기생충 치료를 하세요.\n\n출국 전 30일 이내에 해야 해요.',
        validationIds: ['za.external-parasite-within-30days'],
      },
      // 임상검사 — 출국 전 10일 이내(한국 별지 제25호와 같은 창)라 base 문구의 10일이 그대로
      //   맞다. 남아공 전용 서식을 함께 발급받는다는 점 + **6개월 연속 거주** 확인이 남아공
      //   고유라 문구를 교체한다.
      // 사용자 확정본 3줄(2026-07-31 직접 지정). ⛔ 다듬지 말 것 — 뺀 줄이 둘이다:
      //   · '남아공 건강증명서는 수입 허가와 함께 받은 양식을 써요. 일반 영문 건강증명서로는
      //     안 돼요.' → 서류 탭(za-veterinary-health-certificate)이 같은 말을 한다.
      //   · '태어난 뒤 계속 한국에 있었거나, 최근 6개월 동안 한국이나 남아공에 연속으로
      //     있었어야 해요.' — 증명서 3항 거주 이력.
      //     ⚠️ 이 요건 자체는 살아 있다(최근 6개월 안에 제3국에서 한국으로 들어온 동물은 못
      //       채운다). 앱은 거주 이력을 저장하지 않아 **판정할 수 없어** 룰이 없고, 이제 카드
  ...
  //   배선. 광견병 비발생국이라 귀국 항체 면제(항체=입국용). 편도 전용이라 귀국 카드 없음
  //   (구 sg-export-quarantine 은 도달 불가라 2026-08-01 제거 — git history 참조).
  ...
  indonesia: seaPermitOverrides({
    ...
    importQuarantine: {
      ...
      description:
        '인도네시아 도착 후 공항 검역소에서 검역을 받으세요.\n서류를 확인한 뒤 계류시설로 이동해요.\n격리 기간은 보통 7일이고 최대 2주까지 걸릴 수 있어요.',
  ...
  philippines: seaPermitOverrides({
    ...
    importPermit: {
      ...
      attachmentLabel: '필리핀 수입 허가증(SPSIC)',
      validationIds: [
  ...
  hongkong: seaPermitOverrides({
    ...
    importPermit: {
      ...
      attachmentLabel: '홍콩 수입 허가증(Special Permit)',
    },
    ...
    importQuarantine: {
      ...
      descriptionBySpecies: {
        dog: '홍콩 도착 후 공항 화물터미널에서 AFCD 검역관에게 검역을 받아요.\n\n서류와 건강 상태를 확인해요. 이상이 없으면 격리 없이 인도돼요.\n검역을 마친 뒤 등록증(Dog Licence)을 받아야 해요. 수수료는 80홍콩달러예요.',
  ...
  switzerland: {
    ...
    // 항공권 — EU 공통(채혈 후 3개월)에 스위스 고유 하한이 하나 더 붙는다. 한국은 스위스가
    // 분류한 광견병 위험국이라 **생후 7개월** 미만은 입국 자체가 금지다(BLV). 3개월 대기만
    // 채우면 생후 6개월 20일쯤이라 이 조건이 실제로 더 늦는 경우가 있다 — 둘 중 늦은 날짜가
    // 입국 가능일. 그래서 euFamilyOverrides 의 flight-purchase 를 통째로 덮어쓰고
    // validationIds 에 두 룰을 함께 지목한다(하나만 두면 나머지 경고가 카드 밖으로 샌다).
    'flight-purchase': {
      description:
        '스위스 입국 일정에 맞춰 항공권을 구매하세요.\n\n광견병 항체 검사 채혈일로부터 3개월이 지난 후에 입국할 수 있어요.\n출국일에 생후 7개월 이상이어야 해요.\n반려동물의 스위스 입국은 바젤·제네바·취리히 공항으로만 가능해요.\n항공사에 반려동물 동반 가능 여부를 꼭 확인하세요.',
      cardLine: '스위스에 입국할 수 있어요.',
      earliest: undefined,
      validationIds: ['eu.entry-min-3months-after-titer', 'eu.ch-min-7months-on-departure'],
    },
    ...
    'import-permit': {
      ...
      attachmentLabel: '스위스 수입 허가증(FSVO)',
