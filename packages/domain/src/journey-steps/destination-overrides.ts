...
> = {
  ...
  australia: seaPermitOverrides({
    ...
    extra: {
      ...
      //   ⚠️ 이 문형을 쓰는 곳은 **호주·뉴질랜드 둘뿐**이다(2026-07-30). 어미는 둘 다
      //     '확인하세요'로 통일했다 — 공용 문형 쪽(29곳)은 손대지 말 것. 한 번 전 목적지에
      //     퍼뜨렸다가 되돌렸다.
      'vet-visit': {
        description:
  ...
  new_zealand: seaPermitOverrides({
    ...
    //   문구 구성은 호주 카드를 따른다(2026-07-30 사용자 지시): 인수·이동 → 도착 후 검진 →
    //     계류 기간 → 석방. 호주에서 일부러 뺀 '문제가 있으면 계류가 길어질 수 있어요' 도
    //     같은 이유로 뺐다 — 석방 줄의 '문제가 없으면' 이 같은 사실을 담는다. ⛔ 되돌리지 말 것.
    //   마지막 줄(지방의회 등록)은 호주에 없는 뉴질랜드 전용 사실이라 남긴다.
    importQuarantine: {
      fieldKey: 'nz_import_quarantine_date',
      description:
        '뉴질랜드 도착 후 공항에서 수입 허가에 적힌 MPI 승인 계류시설로 바로 옮겨져요. 이동은 시설이 준비해요.\n\n도착 후 72시간 이내에 MPI 검역관의 검진을 받고, 계류가 끝날 때 한 번 더 받아요.\n계류 기간은 최소 10일이에요.\n서류와 검역에 문제가 없으면 계류 기간이 끝나는 날 보호자나 대리인이 데려갈 수 있어요.\n뉴질랜드에 도착한 뒤에는 거주지 지방의회에 반려견을 등록해야 해요.',
