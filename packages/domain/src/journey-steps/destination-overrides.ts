...
> = {
  ...
  kazakhstan: {
    ...
    //
    // ⚠️ **질병명 정정**(2026-07-22). 구 문구는 '디스템퍼·간염·장염·파보·아데노·렙토스피라'로
    //   6개를 나열했는데, EAEU 통일 수의요건(제40장 계열 — 사용자가 준 40-49 근거)의 원문은
    //   개 = чума плотоядных(디스템퍼)·гепатит(전염성간염)·парвовирусные инфекции(파보)·
    //   аденовирусные инфекции(아데노)·лептоспироз(렙토스피라) **5개**, 고양이 =
    //   панлейкопения(범백혈구감소증)뿐이다.
    //   → '장염'은 파보바이러스 감염(파보바이러스성 장염)을 한 번 더 센 것이라 뺐다. 원문에
    //     없는 항목을 요구하면 보호자가 없는 백신을 찾게 된다.
    'general-vaccine': {
