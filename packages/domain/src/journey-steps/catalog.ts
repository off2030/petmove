...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  },
  {
    ...
    //
    // ⚠️ **발급 시점 — 24시간 규정이 1개월 유효기간보다 우선한다** (2026-07-21 원문 확인).
    //   국경검역법 제25조 4항: "олон улсын мал эмнэлгийн гэрчилгээ ... ачуулахын өмнө
    //   24 цагийн дотор олгосон байна" (선적 전 24시간 이내에 발급돼 있어야 한다)
    //   https://legalinfo.mn/mn/detail?lawId=38
    //   수의증명서 지침(lawId=16207130862051)의 '살아 있는 동물 최대 1개월 유효'와 충돌하는
    //   것처럼 보이지만, 유효기간 상한과 발급 시점은 다른 규정이다. **카드에 '1개월 유효'만
    //   쓰면 "2주 전에 받아도 된다"로 읽혀 무효 증명서를 들고 공항에 가게 된다** — 그래서
    //   1개월 문구를 빼고 24시간을 쓴다. (구 문구가 실제로 그렇게 읽혔다.)
    //
    ...
    //   (국경검역법 8.1.1·29.1.3조).
    description:
      '몽골 출국 전 수출 검역을 받고 국제수의증명서를 발급받으세요.\n\n몽골 수의총국(GAVS)에 미리 연락해 신청 방법과 일정을 확인하세요. 한국에서 받은 수출 검역증명서로 바로 발급되기도 하고, 현지 동물병원 확인서를 따로 요구하기도 해요.\n\n증명서는 출국 24시간 이내에 발급받아야 해요. 미리 받아두면 안 되니 담당 기관과 일정을 맞추세요.',
