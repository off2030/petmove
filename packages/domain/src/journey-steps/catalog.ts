...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  },
  {
    ...
    //      Экспорт, импортын хяналтын хэлтэс олгоно"). → 소관 2원화 의문 해소: 실제 창구는
    //      GAVS(vet.gov.mn)가 아니라 전문검사청 계열이다. 다만 카드에 기관명을 단정해 쓸지는
    //      사용자 판단 대기(잘못 보내면 헛걸음이라 캄보디아 GDAHP 사례처럼 신중히).
