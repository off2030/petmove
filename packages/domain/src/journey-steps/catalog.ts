...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  // 광견병 접종(30) 바로 뒤(38) — 수입 허가(40)·운송 예약(42)·검역시설 예약(44)보다 앞.
  ...
  //   가리킨다. 그래서 허가(40) → 예약(44) 이고, 예약은 허가 **발급** 전에 끝나면 된다.
  //   ⚠️ 그래서 이 카드에는 선행 게이트를 걸지 않는다 — 허가 카드의 게이트는 AIA 하나뿐이다.
  // 운송 예약(42)과 **동시에** '다음 할 일'로 뜬다(2026-07-30 사용자 지정). 둘은 순서가 아니라
  //   교차다 — 이 카드는 희망 입국일이 있어야 문의할 수 있고, 운송 확정은 수입 허가가 나와야
  //   한다. concurrent 는 **쌍의 뒤쪽**에 붙는다(scenario.ts head run 이 mainIdx 뒤를 올린다).
  //   ⚠️ 수입 허가(40) 단계에서는 안 뜬다 — 사이의 운송 예약(42)이 비-concurrent 라 거기서 멈춘다.
  ...
  //   QUARANTINE_START_FIELD_BY_DESTINATION / 이 카드의 완료 판정). 호주·뉴질랜드와 같은 이유.
  {
    ...
    //   ⚠️ '희망 입국일을 정한 뒤 일찍 문의하세요'는 **남았다** — 운송 예약(42)이 이 카드보다
    //     앞인 근거가 이제 이 한 줄뿐이다. 지울 땐 order 도 함께 다시 볼 것.
    description:
