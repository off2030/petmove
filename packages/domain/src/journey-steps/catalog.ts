...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  // 판단 규칙 상세는 메모리 project_return_export_quarantine_rule.
  {
    ...
    shortLabel: '수출',
    description:
      // 문구 골격은 eu-export-cert 를 그대로 따른다(1행 필요성 → 2행 발급 절차 → 3행 기재
      // 요건 → 대체서류). 2행만 그 나라 절차로 교체 — EU=동물병원+공무수의사 인증,
      // 대만=검역소 방문. 3행 기재 요건은 한국 입국 기준이라 두 나라가 같다.
      '한국에 다시 입국하려면 출발하는 나라의 정부가 인증한 한국 입국용 검역증명서가 필요해요.\n대만 검역청(APHIA) 분서·검역참에 반려동물을 데리고 방문해 수출 검역을 받으세요. 검역을 통과하면 수출검역증명서(輸出檢疫證明書)가 발급돼요. 출국 1주일 이내에 처리하고, 미리 예약하는 것이 좋아요.\n마이크로칩 번호와 광견병 항체 검사 결과(0.5 IU/㎖ 이상, 채혈 24개월 이내)가 기재돼야 해요.\n\n다음 경우엔 새로 발급받지 않아도 돼요.\n한국 출국 때 받은 대한민국 수출 검역증명서로 대신할 수 있어요. (마이크로칩 번호가 있고, 광견병 항체 채혈일로부터 24개월 이내여야 해요.)',
    doneSummary: '귀국 서류를 준비했어요.',
    cardLine: '귀국 서류를 준비하세요.',
