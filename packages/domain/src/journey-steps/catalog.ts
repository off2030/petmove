...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  },
  {
    ...
    // ✅ **2단계 구조는 확정**(DPSS 「비상업적 동물 이동 요건」 원문). 거주지 **국가 동물병원**
    //   (державна лікарня ветеринарної медицини)이 임상검사·칩·접종 확인 후 국내용 수의증서
    //   **Ф-1**(제1호 서식)을 발급하고, 그것을 **국경검사부서(ВПІК — Відділ прикордонного
    //   інспекційного контролю)**에서 국제수의증명서로 **교환**한다. Ф-1 자체는 최종 서류가
    //   아니다. 사설 동물병원에서 검사·접종을 했더라도 행정서류는 국가 동물병원을 거친다.
    //   ⚠️ 우즈베키스탄('사설 서류 인정 안 됨')과 문형이 비슷하지만 취지가 다르다 — 여기선
    //     사설 병원 진료가 무효라는 게 아니라 **행정서류 발급 주체가 국가기관**이라는 뜻이다.
    //
    ...
    description:
      '우크라이나 출국 전 수출 검역을 받으세요.\n\n먼저 거주지 국가 동물병원에서 임상검사를 받고 수의증서(Ф-1)를 발급받으세요.\n그 서류를 검역기관(DPSS) 국경검사부서에 가져가면 국제수의증명서로 바꿔 줘요.\n증명서는 5일간만 유효해요. 출국일에 맞춰 발급 날짜를 정하세요.\n모든 국경 통과지점에 검사관이 있는 건 아니에요. 출발 당일 국경에서 받으려 하지 말고, 어디서 받을지 미리 확인해 두세요.',
