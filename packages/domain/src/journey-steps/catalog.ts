...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  },
  {
    ...
    //   구비서류」. **2026-07-22 원문 열람 성공**(직전 조사 때는 TLS 만료로 실패했던 것):
    //   sbb.inspection.gov.mn/хувийн-хэрэгцээнийзориулалтаар-муур/ 원문 확인값 —
    //   - 발급기관: **수도 전문검사청(Нийслэлийн МХГ) 수출입검사과**가 개인 반려동물의
    //     「수의·위생 수출증명서」를 발급한다("Нийслэлийн мэргэжлийн хяналтын газрын
    //      Экспорт, импортын хяналтын хэлтэс олгоно").
    //     ⚠️ **소관 2원화는 해소되지 않았다 — 오히려 두 원문이 서로 다른 기관을 지목한다.**
    //      이 페이지는 МХГ(전문검사청) 수출입검사과가 '올гоно(발급한다)'고 명시하는데,
    //      수의증명서 지침 3.1 은 발급 주체를 **수의기관의 국가검사관**(улсын байцаагч)으로,
    //      4.7 은 МХЕГ 를 서명·직인 대조본을 '통보받는 곳'으로 둔다. 어느 쪽이 개인 반려동물의
    //      실제 창구인지 확정 못 했다 → **카드에 기관명을 단정해 쓰지 말 것**(헛걸음 위험).
    //   - 구비서류: 신청서(국경통과지점·시기·경로 방향·운송수단 종류·주소·전화번호 명기),
    //     반려동물 여권·건강수첩 사본, 소유자 해외여권 사본. 상업 목적이면 매매·증여 계약서 추가.
    //   - **공증 불필요**("Баталгаажуулах шаардлагагүй"), **수수료 10,000 투그릭** 납부 후 영수증 첨부.
    //   - 신청서 서식 PDF 가 같은 페이지에 게시돼 있다(122KB, 열람 확인).
    // ⚠️ **이 페이지는 링크로 걸 수 없다** — Let's Encrypt 인증서가 2026-01-04 만료된 채라
    //   브라우저가 경고 인터스티셜을 띄운다. 내용은 신뢰하되 링크는 걸지 않는다.
    ...
    //   - 별도 협정이 없으면 **몽골어와 영어**로 작성된다(4.5 — 수출용만. 수입용은 몽골어).
    //   - 4.6: 총 3부를 만들어 1부 신청인·2부 **세관**·1부 보관. 여행객용은 2부 인쇄.
    //     → 본문에서 '2부를 받아요'를 뺐다(2026-07-22). 2부 중 1부는 세관으로 가므로
    //       고객이 2부를 챙겨 간다는 뜻이 아니다. 부수는 아예 쓰지 않는 쪽을 택했다.
    //   - 살아 있는 동물의 국제수의증명서는 발급일부터 **최대 1개월** 유효(4.8). 국내용
    //     수의증명서는 30일(8.1.1).
    //     ⛔ **이 1개월을 카드에 쓰면 안 된다**(2026-07-22 결론). 상위법이 정반대로 빡빡하다 —
    //       아래 국경검역법 25.4 참고. '1개월 유효하니 일정에 맞춰 받으세요'라고 안내하면
    //       고객이 일주일 전에 발급받고 법 요건을 위반한다. 본문에서 삭제했다.
    //     ⚠️ 옛 주석의 "전문검사청 안내의 '30일'과 일치 — 두 출처 교차확인"은 **사실이 아니다.**
    //       원문을 실제로 열어 보니 그 페이지엔 유효기간 언급이 **0회**다(검색 스니펫만 보고
    //       쓴 것). 유효기간 근거는 legalinfo 지침 **단독**이다.
    //   신청 시 제출: 수의증명서·검사결과, 신분증 사본, 동물 종류·수·식별정보,
    //     운송수단, 이동경로, 출국 국경검문소·공항, 출국일.
    //
    // ★★ 국경검역법(legalinfo.mn/mn/detail?lawId=38, **2023-12-08 개정**) — 최상위 근거.
    //   사용자가 가져온 조사 자료를 계기로 원문 대조(2026-07-22). 두 조항이 카드를 바꿨다.
    //   - **25.2**: 국제수의증명서는 **동물보건 담당 정부행정기관**(= GAVS)이 발급한다.
    //     → 위의 '기관 2원화' 충돌은 이 조항으로 정리된다. 법이 하위 지침·기관 게시물보다
    //       상위이고 개정도 가장 최근이다. 그래서 카드에 **GAVS 를 명시**하기로 했다.
    //       (전문검사청 페이지가 말하는 '수의·위생 수출증명서'는 별개 서류이거나 구식 안내로
    //        보이나 확정하지 않았다 — 어느 쪽이든 최종 서류는 25.2 의 국제수의증명서다.)
    //   - **25.4**: 국제수의증명서는 동물을 **선적하기 전 24시간 이내에 발급**되어야 한다.
    //     → 지침 4.8 의 '1개월 유효'와 정면 충돌하며 법이 이긴다. 실무 함의: 도심 GAVS 에서
    //       **출발 전날 발급**이면 대부분 충족되지만(전날 09시 발급 → 다음날 08시 출발 = 23h),
    //       **금요일 발급 → 월요일 아침 출발 = 62h 로 깨진다.** 카드 마지막 문장이 이 구멍을
    //       겨냥한 것이다. 임의로 지우지 말 것.
    //   - 29.1.2~29.1.4: 국경에서 세관·국가검사관에게 신고하고 증명서를 제시하고 동물을
    //     검사에 내놓을 의무. **공항 창구는 '확인'이지 '발급'이 아니다.** GAVS 산하 8개 조직
    //     (vet.gov.mn/contact)에 공항 지점이 없다는 것과도 맞는다. 캄보디아(본부 신청 →
    //     공항 검사 → 발급)와 구조가 반대라 문구를 복제하면 안 된다.
    // 채택하지 않은 것(같은 조사 자료에서 — 근거가 모자라 뺐다):
    //   · **'출국 20일 전 신청'** — 29.1.1 이 23.1 의 기한을 수출자에게도 걸지만 23 조 제목이
    //     'Импортын мэдэгдэл(수입 통보)'이고 23.1.4 의 20일도 수입 문맥이다. 문언이 모호해
    //     숫자를 넣지 않는다(베트남에서 서식번호·유효기간을 뺀 것과 같은 기준).
    //   · **ICertificate 온라인 신청**(ivc.mahis.gov.mn, 살아 있음) — 개인 반려견이 전 과정
    //     온라인으로 끝나는지, 동물을 어디에 제시하는지가 공개 화면으로 확인되지 않는다.
    // TODO 후보: 24시간 규정을 검증 룰로 만들지 검토. 지금은 문구로만 안내한다
    //   (mn.export-quarantine-date-valid 는 '몽골 체류 중인가'만 본다).
    // ⚠️ 아래 본문은 **사용자가 직접 불러준 문구 그대로**다(2026-07-22 최종안). 캄보디아와
    //   같은 규칙 — 안전문장 추가·동사 변경·문단 병합 금지.
    //   어미만 앱 통일선인 해요체로 바꿨다('발급받아야 합니다/필요할 수 있습니다' → '해요/있어요').
    //   베트남 카드도 같은 처리를 했다(그 카드 주석 참고). 내용은 한 글자도 안 바꿨다.
    // ⛔ **의도적으로 뺀 것 — 되살릴 땐 사용자에게 먼저 물을 것:**
    //   · 선적 전 24시간 발급 규정(법 25.4). 근거는 확실하지만 그 앞의 '어느 창구에서 언제
    //     받는지'가 공개 자료로 확정이 안 돼, 시간 제약만 남기면 고객이 대비할 방법 없이
    //     불안해지기만 한다는 판단(사용자 결정). 근거·실무 함의는 위 ★★ 주석에 남아 있다.
    //   · 공항에서는 발급하지 않는다는 서술, 동물병원 서류만으론 안 된다는 서술.
    //   → 카드가 '미리 GAVS 에 연락하라'로 수렴한 이유다. 창구·시간·주말 발급 여부가
    //     기관 확인으로 밝혀지면 그때 단계별로 쪼갠다.
    // 연락처·주소를 본문에 쓰지 않는 것도 의도다(사용자 지정) — 아래 links 의 GAVS 공식
    //   연락처 페이지가 주소·부서별 전화·업무시간을 다 담고 있어 중복이고, 사이트가 바뀌어도
    //   링크는 최신을 가리킨다. 캄보디아·베트남이 연락처를 본문에서 뺀 것과 같은 이유.
    description:
      '몽골 출국 전 수출 검역을 받으세요.\n\n몽골 수의청(GAVS)에 미리 연락을 해서 국제 수의증명서를 발급받아야 해요.\n\n국제 수의증명서 발급을 위해 현지 동물병원이 발행한 접종 및 건강증명서가 필요할 수 있어요.',
    ...
    links: [
      {
        url: 'https://vet.gov.mn/contact',
        label: '몽골 수의청(GAVS) 연락처',
      },
    ],
  },
  {
