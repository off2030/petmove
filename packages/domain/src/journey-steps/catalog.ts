...
export const JOURNEY_STEP_CATALOG: StepDefinition[] = [
  ...
  },
  {
    ...
    //        실무상 가장 중요한 값이라 카드 마지막 문단에 쓴다.
    //   §50: 증명서는 "최종 목적지 도착 시까지 유효" — **발급 후 N일 규정이 없다**(몽골과 다름).
    //   §43~45: 주/시 수의국이 **소유자 입회 하에 직접 심사**한다. 민간 동물병원 증명서는
    //        법령에 언급이 없다 → '필수'라고도 '불요'라고도 단정하지 않고 베트남과 같은
    //        '필요할 수 있어요' 헤지를 쓴다.
    ...
    description:
      '우즈베키스탄 출국 전 수출 검역을 받고 수의증명서를 발급받으세요.\n\n거주 지역 수의·축산개발국에 신청하고 출국 공항 국경수의검문소에서 받아요.\n수출 검역을 위해 현지 동물병원 건강증명서가 필요할 수 있어요.\n\n심사에 영업일 7일이 걸리니 출발에 임박해서 신청하지 마세요.',
