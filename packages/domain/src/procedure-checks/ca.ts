...
export const CA_CHECKS: ProcedureCheck[] = [
  ...
  // ── 현지 수출 검역 (왕복 귀국 전) ──
  {
    id: 'ca.export-quarantine-date-valid',
    country: COUNTRY,
    category: '검역',
    title: '캐나다 수출 검역일',
    description:
      '캐나다 수출 검역일은 캐나다 입국일 이후·한국 귀국일 이전이어야 함. 베트남·태국·필리핀·중국 룰과 같은 모양 — "그 나라에 있는 동안 받았는가"라는 물리적 제약이라 나라별 규정 조사가 필요 없다(판정은 classifyExportQuarantineDate 공용).',
    severity: 'warning',
    addedAt: '2026-07-20',
    run: ({ caseRow, destination }) => {
      const data = (caseRow.data ?? {}) as Record<string, unknown>
      ...
      const ret =
        typeof ctx.data.return_date === 'string' && ctx.data.return_date.length >= 10
          ...
          : ''
      const verdict = classifyExportQuarantineDate(data.ca_export_quarantine_date, entry, ret)
      if (verdict === 'skip') return SKIP
      if (verdict === 'before-entry') {
        return {
          ok: false,
          message: '캐나다 수출 검역일은 캐나다 입국일보다 빠를 수 없어요. 날짜를 확인하세요.',
          offendingPaths: ['ca_export_quarantine_date'],
        }
      }
      if (verdict === 'after-return') {
        return {
          ok: false,
          message: '캐나다 수출 검역일은 한국 귀국일보다 늦을 수 없어요. 날짜를 확인하세요.',
          offendingPaths: ['ca_export_quarantine_date'],
        }
      }
      return { ok: true, message: `캐나다 수출검역일 체류 기간 내.` }
    },
  },
]
