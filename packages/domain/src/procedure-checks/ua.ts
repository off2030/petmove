...
export const UA_CHECKS: ProcedureCheck[] = [
  ...
  // ── 광견병 ──
  {
    ...
    description:
      'SSUFSCP: "Вакцинація проти сказу здійснюється починаючи з 12-тижневого віку" (12주부터) = 생후 84일(프로파일 minAgeDays). 저장 거부(카탈로그 earliest 파생)와 같은 함수(validateRabiesPrimeAgeForDestination). 구 blanket 보수 기준(91일 AND 캘린더 3개월)은 규정보다 과잉이라 정확화(2026-07-25 ③).',
    // severity 승격 info→warning(2026-07-21) — 구세대 파일의 잔재였다. 의료·일정 룰은
    // warning 이 앱 기준이다(다른 목적지와 동일). 이 룰은 카드에 매핑돼 있어 배지가 카드에 붙는다.
    severity: 'warning',
    addedAt: '2026-05-07',
    run: ({ caseRow }) => {
      ...
      const first = rabies[0]
      const msg = validateRabiesPrimeAgeForDestination(birth, first.date, 'ukraine')
      if (msg) {
        return {
          ok: false,
          message: msg,
