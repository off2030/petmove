  ...
  // ── 구충 ──
  {
    ...
    run: ({ caseRow, destination }) => {
      ...
      if (diff > 14) {
        return {
          ok: false,
          message: `외부구충(${latest.date})부터 출국일(${dep})까지 ${diff}일입니다. 출국 포함 15일 이내(14일 전 이후)여야 합니다.`,
  ...
  },
  {
    ...
    run: ({ caseRow, destination }) => {
      ...
      if (diff > 14) {
        return {
          ok: false,
          message: `내부구충(${latest.date})부터 출국일(${dep})까지 ${diff}일입니다. 출국 포함 15일 이내(14일 전 이후)여야 합니다.`,
