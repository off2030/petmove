...
export const MX_CHECKS: ProcedureCheck[] = [
  ...
  // ── 광견병 ──
  {
    ...
    description:
      'SENASICA: "Pets under three months of age are exempt" — 달력 3개월 기준(프로파일 minAgeMonths, 원문에 91 days 표현 없음). 저장 거부(카탈로그 earliest 파생)와 같은 함수(validateRabiesPrimeAgeForDestination). 구 blanket 보수 기준(91일 AND)은 정확화(2026-07-25 ③).',
    ...
    run: ({ caseRow }) => {
      ...
      const msg = validateRabiesPrimeAgeForDestination(birth, first.date, 'mexico')
      if (msg) {
        return {
          ok: false,
          message: msg,
