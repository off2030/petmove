...
const DATE_SAVE_BLOCK_DECISIONS: Record<string, string> = {
  ...
  //   '논리적으로 불가능한 조합'이 아니기 때문). 전염병 검사는 이 논리를 따르지 않는다 —
  //   차단 확정(2026-08-01 사용자 결정), 아래 infectious-disease-test 항목 참고.
  'civ-vaccine': '주의만: au.civ-14days-before-departure·au.civ-within-12months — 날짜 수정으로 회복 가능한 위반이라 기록을 막지 않는다',
  // ✅ 차단 확정(2026-08-01 사용자 결정) — "45일 창 밖이면 어차피 호주 입국이 안 되고,
  //   입력하고 싶으면 출국일을 변경하는 게 맞다." CIV(주의만)와 갈리는 근거: CIV 는 재접종으로
  //   회복 가능한 개별 요건이지만, 전염병 검사 창은 입국 성립 자체의 전제라 창 밖 기록을
  //   '유효한 준비'처럼 남겨두지 않는다. 해소 경로 = 출국일 변경 또는 재검사 후 새 날짜 입력.
