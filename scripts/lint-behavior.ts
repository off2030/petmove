...
/**
 * 실행 타임존 고정 — 골든은 KST 기준으로 만들어졌다.
 *
 * NOW 는 절대 시각이라 타임존과 무관하지만, 도메인이 '오늘/내일'을 판정할 때 로컬 날짜를
 * 타므로 UTC 머신에서는 예정일 알림이 하루 어긋나 결과가 달라진다. 실제로 로컬(KST)에서만
 * 통과하고 CI(UTC)에서 실패했다 — "테스트 마이크로칩 삽입 예정일" 알림이 UTC 에서만 떴다
 * (2026-09-05, 이 검사를 CI 에 편입하면서 드러남).
 *
 * import 는 hoist 되지만 어떤 모듈도 로드 시점에 타임존을 캡처하지 않으므로, 여기서 한 번
 * 박아두면 이후 모든 Date 연산에 적용된다.
 */
...
const SCENARIOS: Scenario[] = [
  ...
  {
    name: '출국일에 생후 7개월 미만',
    why: '광견병 접종·항체 30일·채혈 후 3개월을 전부 채웠는데도 나이만 모자란 케이스 — 스위스(BLV 위험국발 7개월)만 걸려야 한다. EU 3개월 대기로 잡히는 것과 다른 규칙이라 대기를 채운 상태로 확인한다',
    departure: '2026-09-25',
    data: {
      birth_date: '2026-03-01',
      microchip_implant_date: '2026-05-10',
      // 생후 85일 접종 — 12주(84일) 하한 통과.
      rabies_dates: [{ date: '2026-05-25' }],
      // 접종 +31일 채혈, 채혈 +3개월 = 2026-09-25 = 출국일(대기 충족).
      rabies_titer_records: [{ date: '2026-06-25', result: '1.2' }],
  ...
  },
  {
    ...
    data: {
      ...
      entry_date: '2026-12-01',
      return_date: '2026-12-20',
      // 대만 20일 마감 지남. 태국 9일 마감도 지남.
      import_permit_application_date: '2026-11-25',
    },
  },
  {
    // 20일 이상 구간 — 마감 이전의 정상 신청. 구 120일 격리 안내 주의는 2026-08-04 제거
    // (120일은 마감이 아니라 격리 면제 경로 중 하나 — 면제 안내는 카드 설명문 담당).
    name: '수입허가를 도착 60일 전에 신청 (20~120일)',
    why: '20일 마감 이전의 정상 신청 — 대만은 이제 아무 주의도 뜨지 않아야 한다(구 120일 격리 안내 제거)',
    departure: '2026-12-01',
    data: {
      birth_date: '2024-01-01',
  ...
  // ── 미국 전용 분기 ────────────────────────────────────────────────────
  {
    name: '미국 — 생후 6개월 미만 강아지 편도(출국일 기준 판정)',
    why: '생후 6개월 판정은 출국일 기준이라, 옛 케이스에 남은 도착일(6개월 넘김)이 있어도 저장 거부·주의가 그대로 떠야 한다',
    destinations: ['usa'],
    tripType: 'one_way',
    showSteps: true,
    departure: '2026-06-01', // 생후 5개월 — 차단 대상
    data: {
      species: 'dog',
      birth_date: '2026-01-01',
      // 항공권 카드에서 도착일 입력은 없앴다(2026-07-26). 과거에 저장된 stale 값이 판정을
      // 가로채지 않는지 확인하려고 **통과할 법한 날짜**를 일부러 남겨 둔다.
