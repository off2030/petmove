...
/** 검사 탭 하단 배송서류 다이얼로그 — 튜브 갯수 + 수신 실험실 + 종 + 발송일 입력. */
...
function ShipmentDocsDialog({ onClose, onSubmit }: ShipmentDocsDialogProps) {
  ...
  // ARC-OVI(남아공)는 표준 16점 혼합 검체라 검체수 고정.
  ...
  const labs = [
    ...
    { value: 'ksvdl', label: 'KSVDL' },
    { value: 'vbddl', label: 'VBDDL' },
    { value: 'arc_ovr', label: 'ARC-OVI (남아공)' },
  ]
  ...
  function handleSubmit() {
    // ARC 는 16점 고정 — 서버에서도 강제하지만 UI 도 16 으로 전달.
    const n = isArc ? 16 : Math.trunc(Number(tubeCount))
        ...
        <div className="mb-5">
          ...
          {isArc ? (
            <div className="text-sm text-foreground">
              ARC 표준 <span className="font-medium">16점 (EA)</span>
              <span className="block text-[12px] text-muted-foreground mt-0.5">혈청 3ml×3 · EDTA 0.5ml×10 · 무염색도말×3</span>
            </div>
          ) : (
