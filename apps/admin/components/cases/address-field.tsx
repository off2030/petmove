...
export function AddressField({
  ...
  const inputClass =
    'flex-1 min-w-0 h-8 max-w-[480px] rounded-md border border-muted-foreground/60 bg-background px-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring/30'
          ...
          {editingKr ? (
            <div className="flex items-center gap-sm flex-1 min-w-0">
              ...
              <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={saveKr} disabled={saving}
                className="shrink-0 whitespace-nowrap inline-flex h-7 items-center rounded border px-2 text-[11px] border-pmw-accent bg-pmw-accent/15 text-pmw-accent-strong hover:bg-pmw-accent/25 transition-colors disabled:opacity-50">
                {saving ? '...' : '저장'}
              </button>
