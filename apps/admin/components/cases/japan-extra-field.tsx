...
export function JapanExtraField({ caseId, caseRow, sectionNumber }: { caseId: string; caseRow: CaseRow; sectionNumber: string }) {
  ...
  async function handleFiles(files: File[]) {
    ...
    for (const file of extractable) {
      uploadFileToNotes(caseId, caseRow, file, updateLocalCaseField, { label: '일본 추가정보' }).catch(() => {})
    }
...
function JapanTextRow({ label, value, placeholder, editing, onStartEdit, onSave, onCancelEdit, type = 'text' }: {
  label: string
  value: string | null
  placeholder: string
  editing: boolean
