...
/**
 * 모바일 이미지 첨부 시 표시되는 자르기(crop) 모달 — 고정 박스 / 줌·팬 방식.
 *
 * react-easy-crop — 크롭 박스는 컨테이너 풀 사이즈 고정, 사용자는 이미지를
 * 줌/팬해서 잘릴 영역을 맞춤. 백신·구충 카드처럼 작은 영수증성 문서를 빠르게
 * 잘라내는 데 유리. 자유 박스 조정이 필요한 경우엔 ScanFlow(자유 크롭) 사용.
 */
...
interface ScanFlowEasyProps {
  source: File
  onConfirm: (file: File) => void
  onClose: () => void
}

export function ScanFlowEasy({ source, onConfirm, onClose }: ScanFlowEasyProps) {
