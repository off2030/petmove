...
export async function getSettingsBootstrap(): Promise<SettingsBootstrap> {
  const [companyInfo, orgType, membersRes, invitesRes, superAdminsRes, vaccineRes, autoFillRes, myProfile, myRole, detailViewSettings, destinationOverrides] = await Promise.all([
    getCompanyInfo(),
    getOrgType(),
    listMembers(),
    listInvites(),
  ...
  return { companyInfo, orgType, members, invites, superAdmins, vaccineProducts, autoFillRules, myProfile, myRole, detailViewSettings, destinationOverrides, errors }
}
