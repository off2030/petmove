...
function resolveField(
  ...
  // `serial_with_expiry` — 호주·뉴질랜드 한정으로 batch + 제품 유효기간(제품 사용기한)을
  // 한 칸에 병기 (e.g. "G98321 / 2027/10/07"). 면역유효기간은 별도 1Y/2Y/3Y 체크박스가 담당.
  ...
  //  - 날짜: raw(채혈일=infectious_date:arc_ovi, 없으면 내원일). YYYYMMDD(구분자 없음).
  //    채혈일·내원일 모두 없으면 Date 칸과 일관되게 빈 값(오늘로 대체하지 않음).
  ...
  if (transform === 'arc_sender_ref') {
    ...
    return `${prefix}-${md[1]}${md[2]}${md[3]}-01`
  }
  ...
  if (vetMatch) {
    ...
    // vet:name_en_upper — 영문명 대문자. 싱가포르 서식처럼 BLOCK LETTERS 칸용.
    if (key === 'name_en_upper') {
      return String(VET_INFO.name_en ?? '').toUpperCase()
    }
...
export async function fillPdfMulti(
  ...
  // 본병원/타병원 약품 규칙:
  //  - 본병원(other_hospital !== true): 입력값 있으면 사용, 없으면 카탈로그(지정 약품) 폴백.
  //  - 타병원(other_hospital === true): 입력값만, 카탈로그 폴백 X.
  // applyRecOverrides 가 record 별로 단일 처리 — 별도 전처리 불필요.
...
export async function fillPdf(formKey: string, caseRow: CaseRow, options?: FillOptions): Promise<FillResult> {
  const lookups = await getOrgVaccineLookups()
  // 본병원/타병원 약품 규칙은 applyRecOverrides 가 record 별로 처리 — 별도 전처리 불필요.
