...
function collectDeadlineReminders(caseRow: CaseRow, now: Date): AppReminder[] {
  ...
  for (const token of destinationTokens(caseRow)) {
    ...
    if (key === 'japan') {
      ...
      // 대만 수입허가증 — 마감 = 도착 20일 전 단일(2026-08-04: 구 120일 2단계 알림 제거.
      // 120일은 마감이 아니라 격리 면제 경로 중 하나라 마감 알림에서 뺐다 — 면제 조건은
      // 카드 설명문 담당). '7일 격리' 단정도 뺀다 — 20일 전 신청이라도 보고서 직송이면 면제된다.
      if (entry && deriveImportPermitStatus(flat) === 'not_started') {
        const r27 = leadReminder(
          ...
          entry,
          27,
          '대만 수입허가증 신청 마감이 일주일 남았어요. 도착 20일 전까지 온라인으로 신청하세요.',
          now,
        )
        if (r27) out.push(r27)
        const r20 = leadReminder(
          ...
          entry,
          20,
          '오늘까지 대만 수입허가증 신청이 필요해요(도착 20일 전).',
          now,
        )
        if (r20) out.push(r20)
      }
    ...
    //   아랍에미리트·말레이시아와 같은 판단. 계류장(QMS) 예약도 고정 마감이 없어 미등록.
      ...
      // 항체 검사 120일 대기 — 마감 일주일 전 + 당일 2회 안내(2026-07-27 사용자 지정.
      // 당시 대만 수입허가 120일 알림과 같은 구조였고, 그쪽은 2026-08-04 20일 단일 마감으로
      // 바뀌었지만 괌 120일은 여전히 유효한 대기 기산이라 그대로 둔다).
      // 대기를 못 채워도 입국은 되지만 남은 기간만큼 계류라,
