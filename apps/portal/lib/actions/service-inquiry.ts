...
/**
 * 서비스 탭 '카카오톡 문의' — 고객이 문의를 시작하면 펫무브 직영 운영자에게 알림.
 *
 * 폼 없이(로그인 상태라 이름·이메일을 이미 앎) 카톡 채팅을 열기 직전에 호출한다. 운영자는
 * 고객의 카톡 메시지를 받기 *전에* 누가·무엇을 문의하는지 미리 알 수 있다. (카톡 채팅 링크는
 * 앱 맥락을 prefill 할 수 없어서, 그 맥락을 이 알림으로 운영자에게 전달.)
 *
 * 알림 = notifications 테이블 직접 insert (2026-08-05 — 구 봇/system 대화방 폐지).
 * 대상 = 펫무브 직영(platform) org 멤버 전원. best-effort — 실패해도 client 는
 * 카톡 열기를 그대로 진행한다.
 */
