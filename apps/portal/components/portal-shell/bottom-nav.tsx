...
function NavIcon({ name, stroke = 1.7 }: { name: Icon; stroke?: number }) {
  ...
  switch (name) {
    ...
    case 'user':
      // 보호자 + 엎드린 동물 — 기하학적 사인파: 작은 마루(폭 3.3·진폭 .99) → 골
      // (폭 2.8·진폭 .84) → 큰 마루(폭 5.6·진폭 1.68). 각 반파는 사인 근사 cubic
      // (제어점 L/4·3L/4, 오프셋 4A/3)이고 진폭/폭 비를 0.3 으로 통일해 이음매의
      // 접선이 수학적으로 정확히 이어진다(G1). 손그림 흔들림 없는 매끈한 파동.
      return (
        <svg {...p}>
          <circle cx="9" cy="7" r="3.6" />
          <path d="M3 20a6 6 0 0 1 12 0" />
          <path d="M11.3 18.3c.83-1.32 2.48-1.32 3.3 0c.7 1.12 2.1 1.12 2.8 0c1.4-2.24 4.2-2.24 5.6 0" />
        </svg>
      )
  }
}
