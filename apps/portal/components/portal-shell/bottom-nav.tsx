...
function NavIcon({ name, stroke = 1.7 }: { name: Icon; stroke?: number }) {
  ...
  switch (name) {
    ...
    case 'user':
      // 보호자 + 엎드린 동물 — 스케치 기반: 진폭 낮은 물결(사람 몸높이 6 대비 최고 2.5).
      // 눕힌 호리병 실루엣 — 작은 봉우리(1.8) → 잘록한 허리(1) → 길고 낮은 봉우리(2.5)
      // → 긴 테이퍼. 반원 병치가 아니라 접선이 이어지는 완만한 파동 하나.
      return (
        <svg {...p}>
          <circle cx="9" cy="7" r="3.6" />
          <path d="M3 20a6 6 0 0 1 12 0" />
          <path d="M10 20c.4-1.1 1.1-1.8 2-1.8c.9 0 1.2.7 2.2.8c1.4.1 1.4-1.5 3.4-1.5c2 0 3.6 1.1 4.2 2.5" />
        </svg>
      )
  }
}
