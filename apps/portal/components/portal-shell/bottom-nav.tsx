...
function NavIcon({ name, stroke = 1.7 }: { name: Icon; stroke?: number }) {
  ...
  switch (name) {
    ...
    case 'user':
      // 보호자 + 작은 동물 — 소셜 'users'(사람 두 명 겹침) 문법에서 뒤 실루엣을
      // 동물로: 귀 달린 작은 머리 + 부분 몸통 아크(왼쪽은 보호자에 가려진 겹침 표현).
      return (
        <svg {...p}>
          <circle cx="9.5" cy="7.5" r="3.6" />
          <path d="M3.5 20a6 6 0 0 1 12 0" />
          <circle cx="17.4" cy="12.4" r="2.4" />
          <path d="M16.3 10.4l-.8-1.8" />
          <path d="M18.5 10.4l.8-1.8" />
          <path d="M21.8 20v-.3a4 4 0 0 0-3.3-3.94" />
        </svg>
      )
  }
}
