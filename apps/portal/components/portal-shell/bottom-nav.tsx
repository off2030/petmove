...
function NavIcon({ name, stroke = 1.7 }: { name: Icon; stroke?: number }) {
  ...
  switch (name) {
    ...
    case 'user':
      // 보호자 + 엎드린 동물 — 스케치 기반: 작은 머리 봉우리(낮고 좁게)에서 목 골을
      // 지나 크고 긴 등 곡선으로, 접선이 이어지는 물결 하나. 반원 두 개 병치 금지 —
      // 봉우리 높이(머리 2.6 vs 몸통 4.1)와 폭(3 vs 8)을 확실히 다르게.
      return (
        <svg {...p}>
          <circle cx="9" cy="7" r="3.6" />
          <path d="M3 20a6 6 0 0 1 12 0" />
          <path d="M10.2 20c.1-1.6.7-2.6 1.5-2.6s1 1 1.7 1.2c1.1.3 1.5-2.7 3.8-2.7 2.6 0 4.1 1.9 4.4 4" />
        </svg>
      )
  }
}
