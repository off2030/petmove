...
const THAILAND_REVIEWS: Review[] = [
  {
    ...
    name: '박○훈님',
    meta: '콩이 · 태국',
    text: '급하게 태국 발령이 나서 콩이 검역 준비 일정이 빠듯했는데, 마이크로칩 이식부터 광견병 항체 검사, 출국 직전 검역소 방문 타이밍까지 제 일정에 맞춰 스케줄을 짜주셔서 정말 감사했어요. 태국은 서류 하나만 잘못돼도 입국이 거절될 수 있다고 해서 긴장했는데, 챙겨주신 원본과 복사본 서류만 들고 가니 당황할 일 없이 수월하게 입국했습니다. 진심으로 감사드려요!',
  },
  {
    initial: '망',
    name: '이○연님',
    meta: '망고 · 태국',
    text: '고양이라 비행기 안에서 예민하게 굴까 봐 걱정이 참 많았는데, 출국 전 검진 때 상담해 주신 대로 진정제를 처방받아 준비했더니 기내에서 얌전히 잘 와주었어요. 태국 현지 검역소에서도 챙겨주신 서류 그대로 제출하니 쓱 훑어보고 바로 통과시켜 주더라고요. 동물검역 규정이 수시로 바뀐다던데, 전문가에게 맡기길 잘했다는 생각이 들었어요. 정말 감사합니다!',
  },
  {
    initial: '보',
    name: '김○수님',
    meta: '보리 · 태국',
    text: '방콕으로 이주하면서 태국 입국 서류 때문에 막막했는데, 덕분에 무사히 잘 도착했습니다. 수입허가서(Import Permit) 발급이랑 규정에 맞춰 접종 스케줄을 짜는 건 혼자선 도저히 엄두가 안 나더라고요. 출국 전 건강검진부터 광견병 접종 증명서, 출국용 건강증명서까지 완벽하게 준비해 주신 덕분에 현지 공항 동물검역소에서 질문 하나 없이 바로 통과했습니다. 대형견이라 더 걱정했는데 마음이 든든했어요. 진심으로 감사드립니다!',
  },
]
...
const PHILIPPINE_REVIEWS: Review[] = [
  {
    ...
    name: '정○우님',
    meta: '호두 · 필리핀',
    text: '갑작스럽게 클락으로 발령이 나는 바람에 호두 검역 준비 시간이 너무 촉박해서 눈앞이 깜깜했어요. 인터넷을 찾아보니 필리핀은 규정이 까다로워 반려견을 두고 가야 하나 진지하게 고민했을 정도였거든요. 그런데 제 출국 날짜에 맞춰 역산해서 접종이랑 건강증명서 발급 일정을 기가 막히게 짜주셨습니다. 출국 전날 병원에서 마지막 서류 점검을 받을 때 “이대로만 내시면 아무 문제 없습니다” 하셨는데, 클락 공항에 도착해보니 정말 그 말씀이 맞았어요. 완벽한 서류 준비 정말 감사합니다.',
  },
  {
    initial: '루',
    name: '최○진님',
    meta: '루나 · 필리핀',
    text: '루나 데리고 세부에 입국해서 짐 정리하다가 이제야 감사 인사 남겨요. 루나가 워낙 예민한 고양이라 출국 전 병원 갈 때부터 걱정이 많았는데, 검진을 꼼꼼히 해주시고 안심시켜 주셔서 너무 감사했어요. 필리핀은 광견병이나 예방접종 증명서를 깐깐하게 본다고 들었는데, 챙겨주신 서류 봉투 그대로 세부 공항 검역소에 제출하니 질문 하나 없이 패스였어요! 원본이랑 사본 헷갈리지 않게 다 분류해 주신 센스에 진짜 감동받았습니다. 정말 감사합니다!',
  },
  {
    initial: '초',
    name: '박○훈님',
    meta: '초코 · 필리핀',
    text: '초코네 마닐라 무사히 도착했습니다! 필리핀은 동물농업국(BAI) 사이트에서 수입허가서를 받는 과정이 워낙 복잡하고 승인도 늦게 난다고 해서 걱정이 태산이었거든요. 혼자 하려고 했으면 출국 전날까지 피 말렸을 것 같아요. 초코 접종 스케줄부터 마이크로칩, 제일 중요한 수입허가서랑 출국 건강증명서까지 날짜를 딱딱 맞춰 완벽하게 준비해 주신 덕분에 마닐라 공항 검역소에서 금방 통과했습니다. 현지 직원이 서류 파일을 보더니 고개를 끄덕이고 바로 사인해 주더라고요. 정말 감사합니다!',
  },
]
...
const ONLINE_DETAIL: Record<string, DestDetail> = {
  일본: {
    intro:
      '안심하고 준비할 수 있도록 곁에서 도와드려요. 단계별 가이드, 사전 신고, 일본 수출 동물검역 신청 및 예약, 서류 점검을 해 드려요. 준비 중 궁금한 것은 언제든 문의할 수 있어요.',
    included: [
      { label: '단계별 가이드' },
      { label: '사전 신고' },
      { label: '일본 수출 동물검역 신청 · 예약' },
