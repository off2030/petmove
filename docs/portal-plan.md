# 펫무브워크 Portal (B2C 고객용 앱) 개발 계획

**`apps/admin` 의 자매 앱 `apps/portal` — 반려인(보호자)이 자기 케이스를 직접 보고, 입력하고, 추적하는 B2C 셀프서비스 채널.**

상위 SaaS 전환 계획은 [`docs/saas-migration.md`](saas-migration.md) Phase 11. 본 문서는 portal 자체의 비전·범위·데이터모델·마일스톤만 다룬다.
...
### 한 줄 차이
- **admin** (B2B): 병원·에이전시 스태프가 여러 케이스를 처리하는 운영 콘솔
- **portal** (B2C): 보호자가 자기 케이스 1~N건만 본인 시점에서 들여다보는 고객 라운지
...
- ✅ **결제** — 별도 트랙. 시점 미정. portal MVP·11.x 마일스톤과 무관.
