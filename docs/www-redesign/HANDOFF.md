...
**우선순위: ① 글 본문 템플릿(69개 공유) → ② 가이드 세부 다듬기 → ③ 모바일 햄버거 메뉴 → ④ 서비스 드롭다운(운송견적·에이전시, 향후) → ⑤ 랜딩 마감(Alonzo·Next.js 이식·배포·Ghost 이관).**
...
**3. 랜딩 마감 잔여:**
- 실제 **Alonzo 워드마크** 적용(현재 시스템폰트 굵게 임시).
- **`(www)` route group Next.js 이식 + 배포** — base64 대신 next/image, URL 슬러그 보존. (실제 www 접속 최종 단계.)
