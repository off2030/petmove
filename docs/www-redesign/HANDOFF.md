...
- **결정 완료**: 목표=둘 다(앱+직영), 시안2(Warm Bold) 채택하되 **PC는 스플릿**(글 좌 / 사진 카드 우), **모바일은 풀블리드**. 히어로 듀얼 CTA(앱 무료시작 / 전문가 올케어 상담) + "두 가지 방법" 섹션으로 두 수익경로 병기.
- **실물 프로토타입 완료**: `prototype-mobile.html` (생성기 `scripts/make_www_prototype.py`, 사진 base64 인라인). 반응형 브레이크포인트 760/960/1100. 헤드리스 스크린샷으로 검증(edge --headless --screenshot).
- **사진 확정**:
  - 히어로 = 밝은 차 창밖 강아지(Gemini AI 생성 → upscale.media 2배 5056px), `hero-cardog-2x.png`. 폰=세로(강아지 우측)/PC=중앙 카드 별도 크롭. ✦워터마크는 크롭으로 제외. **⚠️AI 이미지 최종 사용 여부는 사용자 재판단 여지 남김.**
  - 밴드 = `patrick-hendry-...unsplash.jpg`(8688px, Unsplash 안전) 3000px 인코딩, PC 세로위치 72%로 사람·개 보이게.
  - 소스 이미지는 사용자 `G:\내 드라이브\PETMOVE\기타\이미지`(생성기 IMG_DIR). Desktop 사본은 한글경로 인코딩 이슈 있어 G: 사용.
- **⚠️ 라이선스 확인 필요(최종 게시 전)**: 히어로=AI(위험 낮음). 밴드=Unsplash(안전). 폴더의 `shutterstock_*`/`iStock-*`는 유료 스톡 → 최종본에 쓰려면 정식 구매 필요.

## 남은 할 일

1. **문구·색·서체 다듬기**: 실제 워드마크 Alonzo 적용, 카피 검토, 신뢰 스트립 숫자(3,200+·100%)는 정직 노선 맞춰 근거값 확보 후 반영(현재 정성표현).
2. **`(www)` route group 로 실제 Next.js 이식 + 배포** (이미지는 base64 대신 최적화 파일로, next/image 반응형). URL 슬러그 그대로 유지.
3. **콘텐츠 이관**(별도, 나중): Ghost Export JSON → MDX 변환 + 이미지 다운로드. 이관 전 **69개 URL 스냅샷** 필수.
