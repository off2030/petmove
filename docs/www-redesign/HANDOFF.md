...
- **판단 항목 전부 해소(2026-07-15 사용자 확정)**: 히어로 shutterstock **라이선스 구매 완료**(이미 구매돼 있었음) / 감성 밴드 협곡 사진 유지 / 신뢰 스트립 "지원 여행지 50+" 라벨 유지(문제없음) / 푸터 대표자명 생략 / 5,300+ 이상무 / "2026년 추가 예정" 11곳 연내 추가 가능 확인.
- **디자인 마무리 리뷰 반영**: `#download` scroll-margin(헤더 가림 6px 교정) · 푸터 전화 tel: 링크화 + © 2026 펫무브(랜딩·하위 공통) · 앱 카드 카피 2건("규정에 맞지 않으면", "놓치지 않게 알려드려요") · 지원 여행지 h2 "앞으로 계속 추가돼요" · 서비스 아이콘 3종 노랑 통일 · 하단 스토어 버튼 흰색 유지(검정 비교 후 기각).
- **문의 페이지 마감**: 채널 아이콘 잉크 검정 · 응대 시간(평일 10:00–18:00, 주말·공휴일 휴진) · 앱 이용 문의 블록(app.petmove.co.kr/support) · 카톡 부제 "펫무브 공식 채널" · 리드 한 줄.
- **가이드 페이지 마감**: 섹션명 "인기 가이드"(일본·싱가포르·호주, 사용자 확정) + Ghost 카드 스타일(요약 2줄+feature image 썸네일, 로컬 ghost-images 참조 — 프리뷰는 content junction 필요) · 리드 "여행지별 검역 준비 방법을 확인하세요" · 검색 빈 결과 안내 · 지역 5그룹 재편(아시아15·유럽6·미주6·오세아니아3(괌 포함)·중동·아프리카4).
- **여행지 글 목록 패널(IA, 사용자 확정)**: 글 2개+ 나라(일본10·미국5·EU3·태국·필리핀·중국·하와이·호주·싱가포르·영국 각2) = 칩 클릭 → 아래 목록 패널(총정리 첫 줄 고정, 칩에 글 수 뱃지, 한 번에 하나 열림, 검색 시 닫힘). 글 1개 나라는 바로 이동. 데이터 = `COUNTRY_POSTS`(제목은 라이브 og:title). Next.js 이식 시 `/guide/<slug>/` 나라 페이지로 승격 예정.
- **고아 글 0 달성**: 라이브 sitemap 69글 전수 대조 — 가이드 허브에서 전부 도달 가능(주제별에 "동물검역 기본"·"한국으로 데려오기" 추가). 이식 후에도 같은 방식(sitemap ↔ 내부 링크 diff)으로 재검증할 것.
...
## ⚠️ 게시 전 체크 — **전부 해소(2026-07-15)**
- ~~**5,300+** 실제 근거 최종 확인~~ → 사용자 확인 완료.
- ~~**히어로 AI 이미지** 최종 결정~~ → 실사 shutterstock_437791615 확정, **라이선스 구매 완료**.
- ~~**푸터**: 대표자명~~ → 생략 확정. (유료 결제 붙이면 통신판매업 신고번호는 그때 추가.)
- ~~**유료 스톡** 정식 구매~~ → 히어로 구매 완료, 밴드=Unsplash(무료).
