...
- **A. 캔버스 색** — ✅ 확정 (2026-07-28): **회색 캔버스 `#F4F6F8` + 흰 서피스**.
  순백 시안은 "flat 과 구분 안 됨, 밋밋함" 사용자 피드백으로 폐기. 함께 확정:
  채팅 패널 = 흰 카드, 흰 서피스에 portal 동일 플로팅 섀도, `--accent`(hover·선택) = 소프트 스카이
  틴트(light `204 100% 94%` / dark `206 30% 18%`) — 선택 행·활성 탭이 브랜드 색을 띠는 게 시그니처.
