...
export function PhilippinesExtraField({ caseId, caseRow }: { caseId: string; caseRow: CaseRow }) {
  ...
  const extra: PhilippinesExtra = (data[DATA_KEY] as PhilippinesExtra) ?? { ...EMPTY }
  ...
  async function saveExtra(next: PhilippinesExtra) {
  ...
  function saveField<K extends keyof PhilippinesExtra>(key: K, value: string | null) {
  ...
  async function tryExtract(input: { images?: { base64: string; mediaType: string }[]; text?: string }) {
    ...
    try {
      ...
      if (result.ok) {
        const merged: PhilippinesExtra = { ...extra }
        ...
        // Arrival airport = inbound (Korea → Philippines)
  ...
  const renderField = (key: keyof PhilippinesExtra, label: string, type: 'text' | 'date' | 'email' = 'text', placeholder = '') => {
  ...
  return (
    ...
    >
      ...
      {renderField('email', '이메일주소', 'email', 'email@example.com')}
      {renderField('address_overseas', '해외주소', 'text', 'Destination address in Philippines')}
      {renderField('postal_code', '우편번호', 'text')}
      {renderField('passport_number', '여권번호', 'text')}
      {renderField('passport_expiry_date', '여권유효기간', 'date')}
      {renderField('arrival_airport', '입국공항', 'text', 'MNL')}
    </div>
  )
}
