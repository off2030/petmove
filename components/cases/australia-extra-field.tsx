...
export function AustraliaExtraField({ caseId, caseRow }: { caseId: string; caseRow: CaseRow }) {
  ...
  const extra: AustraliaExtra = (data[DATA_KEY] as AustraliaExtra) ?? { ...EMPTY }
  ...
  async function saveExtra(next: AustraliaExtra) {
  ...
  function saveField<K extends keyof AustraliaExtra>(key: K, value: string | null) {
  ...
  async function tryExtract(input: { images?: { base64: string; mediaType: string }[]; text?: string }) {
    ...
    try {
      ...
      if (result.ok) {
        const merged: AustraliaExtra = { ...extra }
        ...
        const changed = merged.permit_no !== extra.permit_no
          || merged.id_date !== extra.id_date
          || merged.sample_received_date !== extra.sample_received_date
        if (!changed) {
          setExtractMsg('추출 실패: 호주 관련 정보를 찾지 못했습니다')
        } else {
          const r = await updateCaseField(caseId, 'data', DATA_KEY, merged)
          if (r.ok) updateLocalCaseField(caseId, 'data', DATA_KEY, merged)
          setExtractMsg('호주 정보가 입력되었습니다')
        }
      } else {
  ...
  type StringKey = 'permit_no' | 'id_date' | 'sample_received_date'
  const renderField = (key: StringKey, label: string, type: 'text' | 'date' = 'text', placeholder = '') => {
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 space-y-1">
          {showInput ? (
            <textarea
              ...
              placeholder="허가번호·샘플수령일·ID 확인일 정보를 붙여넣으세요 (Enter로 추출)"
      ...
      {renderField('permit_no', 'Permit No.', 'text')}
      {renderField('id_date', 'ID date', 'date')}
      {renderField('sample_received_date', '샘플수령일', 'date')}
    </div>
  )
}
