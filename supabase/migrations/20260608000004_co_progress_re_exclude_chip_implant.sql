-- =============================================================================
-- 동시 진행 — 마이크로칩 삽입일(microchip_implant_date) 동기화 제외 재적용 (회귀 수정).
--
-- 20260519000004 가 microchip_implant_date 를 nonsync_keys 에 추가했으나, 후속
-- 20260602000001(cascade_history)이 트리거 함수를 통째로 재정의하면서 이 제외를
-- 누락 → 칩 삽입일이 다시 형제 케이스로 전파되는 회귀가 있었다. 삽입일은 아이마다
-- 다른 per-pet 값(칩 번호처럼 동물 고유)이므로 다시 제외한다.
--
-- 20260602000001 의 함수 본문(cascade history INSERT 포함)을 그대로 유지하고
-- nonsync_keys 에 'microchip_implant_date' 만 추가한다.
-- =============================================================================
